            ...
            while (*s) {
                ...
                if (strncmp(s, "use", 3) == 0 && !isalnum((unsigned char)s[3]) && s[3] != '_') {
                    ...
                    if (mod_len > 0 && *s == '.') {
                        ...
                        while (1) {
                            const char* iter_start = s;   // 进度守卫基准：本圈必须推进
                            if (batch_mode) {
                                // 跳过所有空白（含换行，支持跨行 use 语句）。
                                // 只跳空格/制表符会让「use 换行」死循环：s 停在 '\n' 上，
                                // ')' 判断不成立、类型名读到 0 字符、循环体整段跳过，s 一步不动。
                                // （scan_pass2_init.inc 的同名逻辑早已含换行，这里漏了。）
                                while (*s && (*s == ' ' || *s == '\t' || *s == '\n' || *s == '\r')) s++;
                                if (*s == ')') { s++; break; } // 批量模式结束
                            }
                            ...
                            // 批量模式：跳过逗号和空白（含换行，支持跨行 use 语句）
                            while (*s && (*s == ' ' || *s == '\t' || *s == '\n' || *s == '\r')) s++;
                            if (*s == ',') s++;
                            while (*s && (*s == ' ' || *s == '\t' || *s == '\n' || *s == '\r')) s++;
                            // 允许尾逗号
                            if (*s == ')') { s++; break; }
                            // 进度守卫：本圈没推进就退出。畸形列表（如缺逗号）交由真正的
                            // 解析器报语法错误 —— 这里只是别名传导的文本扫描，绝不允许死循环。
                            if (s == iter_start) break;
                        }
                    }
                }
