...
#define CSTRUCT_METHOD_TABLE_MAX_LOAD 0.75

typedef struct CStructMethodHashEntry {
    char* name;
    ObjNative* method;
    int arity;
...
typedef struct {
    CStructMethodHashEntry** entries;
    int capacity;
    int count;
} CStructMethodTable;

static THREAD_LOCAL CStructMethodTable cstructMethodTable = {NULL, 0, 0};

// 计算字符串哈希值（FNV-1a算法）
static uint32_t cstruct_hash_string(const char* str) {
    uint32_t hash = 2166136261u;
    while (*str) {
...
// 初始化 cstruct 方法表
static void cstruct_method_table_init(void) {
    cstructMethodTable.capacity = CSTRUCT_METHOD_TABLE_INITIAL_CAPACITY;
    cstructMethodTable.count = 0;
    cstructMethodTable.entries = (CStructMethodHashEntry**)calloc(cstructMethodTable.capacity, sizeof(CStructMethodHashEntry*));
}

// 释放 cstruct 方法表
static void cstruct_method_table_free(void) {
    ...
    for (int i = 0; i < cstructMethodTable.capacity; i++) {
        ...
        while (entry) {
            CStructMethodHashEntry* next = entry->next;
            free(entry->name);
            free(entry);
            entry = next;
        }
    }
    free(cstructMethodTable.entries);
    cstructMethodTable.entries = NULL;
    cstructMethodTable.capacity = 0;
    cstructMethodTable.count = 0;
}

// 扩容 cstruct 方法表
static void cstruct_method_table_resize(void) {
    int old_capacity = cstructMethodTable.capacity;
    CStructMethodHashEntry** old_entries = cstructMethodTable.entries;
    
    int new_capacity = old_capacity * 2;
    CStructMethodHashEntry** new_entries = (CStructMethodHashEntry**)calloc(new_capacity, sizeof(CStructMethodHashEntry*));
    if (!new_entries) return;
    
    for (int i = 0; i < old_capacity; i++) {
        CStructMethodHashEntry* entry = old_entries[i];
        while (entry) {
            CStructMethodHashEntry* next = entry->next;
            uint32_t hash = cstruct_hash_string(entry->name);
            int index = hash & (new_capacity - 1);
    ...
    free(old_entries);
    cstructMethodTable.entries = new_entries;
    cstructMethodTable.capacity = new_capacity;
}
...
void cstruct_register_method_with_params(const char* name, ObjNative* method, int arity,
    ...
    if (cstructMethodTable.count >= cstructMethodTable.capacity * CSTRUCT_METHOD_TABLE_MAX_LOAD) {
        cstruct_method_table_resize();
    }
    
    uint32_t hash = cstruct_hash_string(name);
    int index = hash & (cstructMethodTable.capacity - 1);
    
    CStructMethodHashEntry* entry = cstructMethodTable.entries[index];
    while (entry) {
        if (strcmp(entry->name, name) == 0) {
            entry->method = method;
            entry->arity = arity;
    ...
    CStructMethodHashEntry* new_entry = (CStructMethodHashEntry*)malloc(sizeof(CStructMethodHashEntry));
    if (!new_entry) {
        native_throw_error("cstruct 方法注册内存分配失败");
        return;
    }
    ...
    new_entry->next = cstructMethodTable.entries[index];
    cstructMethodTable.entries[index] = new_entry;
...
ObjNative* cstruct_find_method(const char* name) {
    if (!cstructMethodTable.entries || cstructMethodTable.count == 0) return NULL;
