...
// ----------------------------------------------------------------------------
// 为什么需要（实测，不是推断）：`new T()` 每次都要真查一次定义表（没有内联缓存）⇒
// 定义表一大，线性扫描的成本就会吃掉全部时间。探针（200 个 struct 声明、循环
// 300 万次 `new T199()`）：1 个定义 215ms → 200 个定义 6250ms（**29×**）。
...
void struct_def_register(ObjStructDef* def) {
    ...
    //    裸名查找按"先注册者"返回，给尚未迁移的调用点（cast / face / 原生桥接）用，
