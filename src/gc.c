...
// 实测表现为「热循环里每轮新键的字典写慢 10 倍」（§8.70 副产品 2：一次**回边安全点**
// 触发的回收把整批年轻对象晋升并全部入集 ⇒ 集合膨胀 ⇒ 后续每次 dict 写都 O(|set|) 扫描）。
...
// 语义：变量存在且首字符不是 0/n/N/f/F 即为开（与 LENO_* 系列开关的判断风格一致）。
 ...
 * ⚠ 旧结论"放大池只省 ~4ns"（§8.31）是在**回边安全点落地之前**测的 —— 那时循环内
 * 分配的对象等不到回收、不会回到 free-list，池必须覆盖整段分配才有效；
 * §8.37/§8.38 落地回边安全点后，回收会周期性发生 ⇒
 * 池只需覆盖**一个回收间隔**，所以放大池立刻见效。 */
 ...
 * 才有效"成立于"循环内分配的对象不回收到 free-list"的前提；§8.37/§8.38 落地
 * **回边安全点**之后，回收会周期性发生 ⇒ 池只需覆盖**一个回收间隔**内的分配量。 */
...
Object* gc_alloc(size_t size, ObjType type) {
    ...
    if (gc.enabled && gc.young_allocated + size > gc.young_threshold && !gc.running) {
        gc.deferred_gc = 1;
        /* ★ 为什么只置标志、不当场回收（§8.37）：
         * 回收只发生在**解释器安全点** —— `OP_RETURN` / `OP_RETURN_MULTI`
         * （每 256 次摊销检查一次，见 op_call.inc）以及 GUI 帧间的
         * `gc_try_collect_deferred()`。那里 `vm.stack` 与各帧 locals 一致，
         * `mark_roots` 看得到全部活值；而在分配中途就地回收会打断正在进行的
         * 运算，尚未入根的中间对象会被当垃圾（§8.36 实测过的静默 use-after-free）。
         *
         * 已知边界（实测过，别"优化"掉）：`deferred_gc` 只被"返回 / 帧间"消费 ⇒
         * **只有分配、既没有调用也没有返回**的热循环（如 `for N { new Pair(..) }`）
         * 会一路涨到函数返回才回收（§8.31 实测堆涨到 ~960MB）。要让它中途也能回收，
         * 正确做法是**让回边也查回收请求**（回边目前只查 `vm.gc_force_request` 这个
         * 测试钩子，见 op_jump.inc / op_for_loop.inc），而不是把回收挪进分配点。 */
    }
    ...
    if (!obj && gc.enabled && !gc.running) {
        /* ★ 分配失败 → 先做一次 Major 回收再试（§8.36 的教训在这里落地）：
         * 这是**唯一**的"同步回收"入口，触发条件是池与系统 malloc 都拿不到内存的
         * 极端时刻。走到这里时没有半构建对象（本次分配尚未成功），活值都在
         * vm.stack / 各帧 locals 上 ⇒ mark_roots 能看到它们。
         * ⚠ 千万不要把它挪到"常规分配路径"上当回收手段：在运算中途回收，尚未
         * 入根的中间对象会被当垃圾（§8.36 实测过的静默 use-after-free）。 */
        {
            gc_major_collect();
            obj = (Object*)malloc(size);
            if (!obj) {
...
//
// 调用点必须是解释器安全点（OP_LOOP / OP_FOR_LOOP / OP_RETURN / OP_RETURN_MULTI）：
// 那里 vm.sp / 各帧 locals 都是一致状态，mark_roots 能看到全部活值。
// ⚠ 不要在非安全点（分配中途、运算中途、native 回调里）新增调用点。
//
...
void gc_force_collect(void) {
    ...
    /* 与 gc_try_collect_deferred 一样，只在解释器安全点被调用（活值全可见 ✓）。
     * 这里额外清掉 deferred_gc：本次已经回收过了，别让标志残留再触发一次无用回收。 */
    gc.deferred_gc = 0;
    gc_minor_collect();
...
static void mark_roots(void) {
     ...
     * 用途：定位「某个活对象到底靠哪个根活下来的」——一次真实的漏标记排查用过
     * （结论：漏标的对象是因为它只存在于**非根**位置，而不是某个根没扫）。 */
    int _mk = 0;
...
void gc_try_collect_deferred(void) {
    ...
    /* ★ 这里可以做**就地**回收，唯一的前提是：调用点必须是"活值全部可见"的位置。
     * 现在的调用点是：a) 解释器安全点 `OP_RETURN` / `OP_RETURN_MULTI`（每 256 次摊销，
     * 见 op_call.inc）；b) GUI 事件循环在帧间（draw callback 返回后）。
     * 两者都不在执行中的运算中途 ⇒ `vm.stack` 与各帧 `frame->locals` 是一致的，
     * `mark_roots` 能看到全部活值。
     *
     * ⚠ 历史教训（Bug5，代价是一整天的调试）：这个函数**曾经**被"调用方还活着"的
     * 情形穿过 —— 当时有另一种执行态（**机器码帧**）持有活值，而 `mark_roots` 只能
     * 扫 `vm.stack` 与各帧 `locals`、看不到那些活值 ⇒ 就地回收把仍在使用的对象当
     * 垃圾，表现为"某个函数每 256 次返回时就出错"（静默 use-after-free）。
     * ⇒ **将来若再引入任何"活值不在 vm.stack / frame->locals 里"的执行态，
     *    必须在这个入口加守卫**（那时回收只能延后到该执行态退出之后）。 */
    gc.deferred_gc = 0;
