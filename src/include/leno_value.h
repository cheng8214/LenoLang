...
typedef struct {
    ...
    TypeKind* return_types;    // 返回值类型数组（编译期从声明提取，长度=return_count；
                               // return_count<=0 时为 NULL。编译期已知，供调用点/工具链消费）
} ObjFunction;
...
typedef struct {
     ...
     * 何时决定回收」，不新增 GC 调用点 —— 即不会在「活值不在 vm.stack /
     * frame->locals 里」的位置触发回收（那是 §8.31 P1 的范围）。 */
     ...
     * 用途：定位「某个活对象到底是靠哪个根活下来的」——一次真实的漏标记排查用过。 */
    int mark_new;
...
/* §8.133：**免装箱**的 int64 操作数快路径 —— 语义与
 * `bigint_x(bigint_from_int64(v))` 完全相同 ✓（内部走同一段代码 ✓），
 * 只是省掉为 int64 操作数分配临时 bigint 的 gc_alloc + malloc ✓。 */
...
// 旧写法会把合法成员误报成「没有成员」（历史上多个调用方都这么判）。
