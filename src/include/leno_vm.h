...
// ============================================================================
// ⚠ 关于本仓库注释里大量出现的「栈式」——**总说明，只需在这里读一次**：
//   栈式 VM 已**全面弃用**，逻辑已并入本仓库的寄存器式实现，且**不再与栈式逐行对照**
//   （见 docs/寄存式与栈式的差异清单.md 的归档说明）。
//   ⇒ 文中「栈式」一律只作为**历史来源依据 / 语义口径对照**保留：
//      · "对齐栈式 OP_CMPJMP_*"、"与栈式 op_arith.inc 同口径" = 这条特化/融合的**来处**
//        与取舍理由（有价值，别删 ✗）；
//      · "栈式仍错 / 两边都错" = **历史注记**（当年两侧的对比结论），
//        **不是**"还有另一份实现要保持同步"的意思 —— 需要改就改本处实现 ✓。
// 编码形态：
...
typedef enum {
    ...
    // 立即数下标读（对齐 Lua 5.5 的 `OP_GETI`：小下标直接编在指令里）：
    //   `arr[0]` 原先是「LOADI tmp,0 + OP_INDEX_ARRAY_INT」两条派发，下标那个临时寄存器
    //   纯属为了凑一条寄存器操作数 —— 现在 C 字段就是字面量下标，一条搞定。
    //   与 OP_INDEX_ARRAY_INT/_FLOAT 是**同一份逻辑**（那两个 handler 本来就逐字相同），
    //   所以一条指令同时服务 Array[int] 与 Array[float]。
    //   ★ C 是**无符号 0..255**，与 Lua 同口径（`lcode.c:isCint()` 用 `l_castS2U` 比
    //     `MAXARG_C` = 255 ⇒ 负字面量下标两侧都不走立即数）。Leno 的负下标本就必然越界，
    //     取无符号反而把 128..255（查找表 / 定长数组）这段纳进来。
    //   ⚠ codegen 只在「数组静态类型是 Array[int]/Array[float]、下标是 [0,255] 整数字面量」
    //     时才发；变量下标、超出 255 的字面量（`arr[300]`）仍走 LOADI + 寄存器版，行为不变。
    //   ⚠ 4 字节定长（无附加数据），追加在末尾 ⇒ 既有 opcode 编号不动，旧 .lenb 仍可执行。
    OP_INDEX_ARRAY_IMM,  // iABC   R[A] = R[B][C]，C ∈ [0,255]

    // for 容器迭代的条件融合（T13）：`idx < len(容器)` + 条件跳转（可选"先自增索引"），
    //   把原先的 `OP_LEN + OP_LT + OP_JMP_IF_FALSE`（回边另有 `OP_INC + OP_JMP`）
    //   合成**一条** —— 每轮固定开销由 6 条降到 2 条。
    //   A = 索引寄存器，B = 容器寄存器；
    //   C 只当标志：bit6 = 真则跳（回边用），bit5 = 先自增索引再测试（回边用）；
    //   紧随 4 字节字的**低 16 位** = 跳转偏移（offset + 32768，与 CMPJMP 同约定）⇒ 共 8 字节。
    //   ★ 关键：长度仍在**每次测试时**读取（与 OP_LEN 逐支同口径，非容器 ⇒ 0）⇒
    //     `for a to x { a.add(..) }` 这种"迭代中增删容器"的语义**一字不变**
    //     （实测：2 元素起跑、每轮追加一个，长度每轮重算 ⇒ 6 轮结束、最终 len=6；
    //      把长度提到循环外会静默变成 2 轮 —— 那才是语义改动，不做）。
...
// （见 docs/函数调用性能优化_调用帧与多返回值解构.md）。
// ---------------------------------------------------------------------------
// 32 → 64（2026-09-22，i5-14400F；依据见 docs/待办与路线图.md ⑤-o）：
//   槽位数是**寄存器高水位**（`func->local_count = gen->max_reg`，含表达式临时寄存器），
//   不是"声明的局部变量个数" —— 所以比直觉更容易越过阈值。实测语料里
//   `光线追踪对象版.leno` 的 `trace` 是 **37 槽**、`全部测试` 的 `main` 是 44 槽，
//   都在 33..64 这个"32 不够、64 够"的区间里，每次调用都要 malloc+free（约 30ns）。
//   **实测（同机同轮、双向 A/B）**：
//     · 大帧深递归（34 槽函数 × 40 层 × 20 万轮）：**1515 → 1024ms（−32.4%）**
//     · `arr32` 微探针（34 槽平调用 300 万次）：**487 → 425ms（−12.7%，每次 −20.7ns）**
//     · 光线追踪 Phase C（`trace` 37 槽）：1691 → 1666ms（−1.5%；每次调用本来要 1127ns，
//       省下的 20ns 占比自然小）
//     · 回退面：小帧深递归 +0.6%、光追 Phase A/B/B2 ±2%、`全部测试` 36 项全在 ±4.6% 内
//       （多数 ±2%）⇒ **没测到回退**。
//   代价：`CallFrame` 384 → 640 字节。帧池是 realloc 增长、按**历史最大深度**算，
...
struct Symbol {
    ...
    // ★ 编译期专用：「该变量的值**确定为 null**」
    //   —— 声明时初始化表达式就是 `null` 字面量，且此后**再没出现过任何写操作**
    //      （`a = v` / `a += v` / `a++` … 都会把它清 0）。
    //   用途：`null 不能参与算术运算` 这条规则里**编译期就能确定**的那一半 ——
    //     `var a = null; a + 1`、`int? a = null; a + 1` 直接报编译错误（教程承诺的语义）。
    //   ⚠ 只对**局部量**置位（全局/模块级变量的写入可能发生在别的函数里，按源码顺序判不安全）。
    //   ⚠ 判据是「宁漏勿误报」：误报会让本来能跑的程序编译不过，比漏报严重得多
    //      ⇒ 只在**能确定**时置位；拿不准（含被别处赋值过）一律不置位，
...
// 几次比较 + 一次取/存。
// 安全性：命中前校验 `dict->entries[slot].key` 与本次键**内容相等**，且 dict 来自**当前帧
//   寄存器** ⇒ 字典一定活着；槽位内容也当场核对 ⇒ 不依赖 GC 保活、也不怕"字典被回收后
//   地址被复用"（那种情况下 capacity/槽位内容核对必然挡住）。写路径**必须保留 GC 写屏障**
//   （与 dict_set 的"更新现有键"分支逐字一致）。
// ⚠ 校验必须用 `dict_ic_key_match()`（vm_helpers.inc；字符串按键内容比），**不能用 `==`**：
//   编译期字面量**不在内化表里**（`intern_table_init()` 只在 vm_init 调用，而编译发生在
//   VM 启动之前）⇒ 源码里两处 `"k"` 是**两个不同对象**，逐位比较会永远不命中（读侧第一版
//   就是这么慢 52% 的，见路线图 ⑤-k/⑤-l）。内化后的键该调用会先走指针相等快路。
// ============================================================================
typedef struct {
