...
// ============================================================================
// §8.133：**免装箱**的 int64 操作数快路径
// ============================================================================
//
// 问题（实测账本，1M 次操作）：大整数运算每操作 **3 次 GC 分配 / 144 字节垃圾**，
// 而 int48 是 **0 次** ✗；crypto 的 45× 差距就出在这里（不是"任意精度运算贵" ✗）。
//
// 原因：`bigint + int48` 原本先把 int48 装箱成 ObjBigInt（`promote_to_bigint`）
// ⇒ 每操作白付「gc_alloc + malloc」两笔 ✗（结果对象当然还要分配 ✓）。
//
// 做法：把 int64 操作数做成**栈上**的 ObjBigInt **视图**（limbs 指向栈数组 ✓），
// 然后调用**同一个** bigint_add / bigint_and / bigint_xor ⇒ 进位、补码、
// 高位裁剪的规则**一字不改** ✓ —— 语义等价由"同一段代码"保证 ✓，不是另写一遍 ✗。
// 该视图**不进 GC**（没有 header 语义、只按引用传递 ✓），被调方只读 ✓；
// 这与既有 `bigint_sub` 里的 `ObjBigInt temp_b = *b;` 是同一套手法 ✓（有先例 ✓）。
//
// 收益：每操作少 1~2 次 GC 分配 + 1~2 次 malloc ✓；若结果落回 int48，则
// `val_bigint_from_limbs` 直接返回 int ⇒ **零分配** ✓（掩码/异或这类最常见 ✓）。
static inline void bigint_view_from_int64(int64_t v, ObjBigInt* view, uint32_t stash[2]) {
    int is_negative = v < 0;
    uint64_t abs_value = is_negative ? (uint64_t)(-(v + 1)) + 1 : (uint64_t)v;
    int count = 0;
    if (abs_value > 0) {
        stash[count++] = (uint32_t)(abs_value & BASE_MASK);
        if (abs_value >> BASE_BITS) {
            stash[count++] = (uint32_t)(abs_value >> BASE_BITS);
        }
    }
    if (count == 0) { stash[0] = 0; count = 1; }
    view->limbs = stash;
    view->limb_count = count;
    view->is_negative = is_negative;   // 与 bigint_from_int64 的规范化一致（0 ⇒ 非负 ✓）
}
