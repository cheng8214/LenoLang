...
## 复核结论（2026-09-18 逐条实测/读码，含"哪些已过期"）
...
| P2 | **已修复** ✓ | 修法（与 `_args()` 对齐成**同一条边界**）：脚本路径**之后**的 `-` 开头参数原样交给脚本；`--` 可显式终止解释器选项；未知旗标写在脚本**之前**仍报错（提示没丢）。实测：`x.leno --list` ⇒ 脚本照跑 ✓、`-- x.leno --list` ✓、`--nope x.leno` ⇒ exit 64 +「未知选项」✓。用例 `assert/test_cli_script_args.leno`（4 判据，含两条守卫）；`module_sys.md` 补了边界规则表。README 里的"位置子命令"绕法现在**不必需**了（但保留兼容 ✓） |
...
| P7 | **已修复** ✓ | 三个 `examples/jsons/*` 示例**全部修好**（**编译 + 真跑**都 exit=0）：新增示例共用的取值层 `examples/jsons/json_any.leno`（`jstr/jobj/jat/jats/jlen/jshow` —— 把"显式类型收窄"收在一处，写法与 `trae_core.leno` 的 `json_get/json_obj` 同源），三个示例改用它；硬编码路径改**仓库相对**；落盘一律改 `_tmpdir()`（不再往仓库根写垃圾）。另修一处既有坑：`var d = {}` 的值类型被**首个插入**推成 `string` ⇒ 之后塞子字典 / `4999` 报「字典值类型不匹配」✗ ⇒ 改显式 `Dict[string, any]` + 子字典先建成变量再整体塞入。**并落地"示例冒烟门禁"**（原文建议）：`assert/examples_smoke.list`（只编译）+ `assert/examples_smoke_run.list`（编译 + 运行）+ `assert/test_examples_smoke.leno`，**15 个示例入册**（12 + 3），**只放本来就绿的**（扩清单 = 承诺不许坏）。顺带扫出 `examples/array/` 仍有 **2 个编译不过**的示例（`方法调用测试.leno`、`测试 map + 匿名函数 .leno`）—— **已登记、未修**，也未入清单 |
| P8 | **已补齐** ✓ | `module_files.md` 写明实测语义：`files.read` 失败**抛运行时错误「无法打开文件」**（带行号）—— **既不是 `""` 也不是 `null`** |
| P9 | **已根治** ✓ | 源清单**只剩三份数据文件**：`sources_core.txt`（52，共享核心）/ `sources_compiler.txt`（33，编译器专属）/ `sources_vm.txt`（1，VM 入口 `vm_main.c` —— 与 `src\main.c` 互斥，必须单独一份）。`build.bat`、`build_vm.bat`、**`build.sh`** 全改成**读它们**（此前三份各自内联 ✗，`build.sh` 的注释甚至写着"以 build.bat 为准"却已漂移）；平台/架构相关的两项（Windows FFI、JIT 后端）不进清单，仍由各脚本按平台追加。两个 `.bat` 现为**纯 ASCII**（`build.bat` 那 1 行中文注释译成英文 ⇒ GBK 陷阱清零）。新增门禁 `assert/test_build_lists.leno`：清单非空 / 路径**真实存在**（改名忘更新即红）/ **不许跨清单重复** / 两个 `.bat` 必须纯 ASCII 且**确实读清单**（防内联回潮）。**验证**：`build.bat` ✓、`build_vm.bat`（console+gui 两个产物）✓、端到端 `leno.exe -c` → `leno_vm.exe` 真跑 `RAN_OK` ✓（证明 VM 清单完整）、门禁 passed（core=52/compiler=33/vm=1）、套件 **332/0**。**未验证**：`build.sh` —— 本机**没有 bash**（改动只是"同读清单"，等价物无法在此自动跑，已如实登记）。**顺带**：非 ASCII 检测**没用 shell**（先写 findstr/grep 版，实测引号玄学 + 把纯 ASCII 文件误判成含非 ASCII ✗），改成纯 Leno 的 `strings.byte_len(s) != s.len()`，并用含中文的 `build.sh` 做**阳性对照** ✓ |
| J1 | **仍暂缓**（未动） | 本文自带量化结论：JIT 39.88 s vs 关 JIT 38.38 s ⇒ 该热路径 **JIT 零收益（反而慢 4%）**；修法方向（`OP_CAST_INT` 的整数来源通道）已写在文末附 B |

**"别改坏的东西"那 5 条**（报错带行列+修法提示、`Bailout: … 触发指令=OP_xxx`、`LENO_*` 开关、assert 自动发现、`§` 编号复盘）在 2026-09-18 复核时**都还在** ✓。
