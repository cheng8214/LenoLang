...
- **（自我更正）曾误判"帧率被 OS 计时器 15.6ms 量化"** ✗ —— 实为**测量方法**问题，不是系统问题 ✓：
  起先我用"外部秒表量 60 帧总耗时 ÷ 60"得到 ≈31.5ms/帧 ⇒ 误判成 tick 网格 ✗。
  真相：`leno.exe` 的**启动耗时 ~0.9s 被算进了帧时间** ✗（每帧虚高约 15ms）。
  **同一次运行内对照**（不受启动影响 ✓）：
    · 60×`SDL_Delay(16)` = 995ms ⇒ **16.6ms/次** ✓（sleep 本来就准 ✓）
    · 60×`SDL_DelayPrecise(16000us)` = 963ms ⇒ 16.05ms/次（略更准，可选 ✓）
  **两次 N 差值法**（20 帧 vs 60 帧 ⇒ 启动耗时互相抵消 ✓）：
    `(1847 − 1201) / 40 = 16.13 ms/帧` ⇒ **62 fps** ✓ ⇒ **设 60 帧确实跑到了 60 帧** ✓
  另外实测 `timeBeginPeriod(1)`（winmm，调用成功 ✓）对本机 sleep **没有收益** ✗ ⇒ 模块里**不做自动调用** ✓。
  **方法论建议（最值得记的一条）**：量 GUI 帧率/单次等待，一律用**两次 N 的差值法**或**同运行内对照** ✓，
  绝不能把进程启动耗时算进去 ✗ —— 我据此误判过一次，写在这里以免后人重踩 ✓。
