...
（⚠ **后续更正（§8.113）**：那枚 `FORSTEP0 = -2000 - off` 基址与"非溢出类"的 `-1000 - off`
**值域重叠** ✗（`off >= 1000` 即撞 ⇒ 误报成 FOR_PREP ✗），已连同对应分支删除 ✓；
且 §8.113 之后那个守卫本身也不存在了 —— `step == 0` 改为**跳过循环**（与解释器同语义）✓。）
...
### 8.113 【已实现】浮点步长 `for` 进 JIT：`FOR_PREP` / `FOR_LOOP` 的 SSE 路径（≈2x，§8.97 的目标）（2026-09-17）

**背景**：§8.97 量到「热循环里包浮点步长 `for`」的净损失 ≈ **2x**（探针 E 175~196ms 与
NO_JIT 158ms 持平 ✗，同形状 int 步长的 F 93~97ms ✓），成因是 `OP_FOR_PREP` 的
**静态守卫**（类型位图 bit=1 ⇒ bailout 交解释器）把整块编译区域拉黑 ✗。

**改法（int 循环零税 ✓）**

| 位置 | 改动 |
| --- | --- |
| `ops_loop.inc` `OP_FOR_PREP` | 位图 bit=1 ⇒ 不再 bail，跳到**新增的 SSE float 路径**（初值 + 初始条件）✓；bit=0 的 int48 代码**一字未改** ✓ |
| `ops_loop.inc` `OP_FOR_LOOP` | 位图 bit=1 ⇒ 新增的 **SSE 回边**（增量 + 方向/含端点比较）✓；int 路径一字未改 ✓ |
| `ops_loop.inc` `OP_FOR_PREP` | `step == 0` 由 **bailout 改成"跳过循环"** ✓ —— 解释器语义本就是 `should_run = 0`（int 0 与 +0.0 裸位型同为 0，语义一致，不必区分）✓；顺带消掉 fm `render` 里那条残留 bailout ✓ |
| `x86_64.c` | `EMIT_MOVQ_RAX_XMM0` 拆出 `_AT(off)` 变体（序言无 `bc_off` 时可用）✓ |

**语义对齐**（逐条对 `src/vm/vminc/op_for_loop.inc`）：`loop_var = start`（**原样搬运**，不改类型 ✓）；
正向 `incl ? start<=end : start<end`、反向 `incl ? start>=end : start>end` ✓；
回边 `loop_var = val_num(loop_var + step)`（结果一律写**裸 double** ✓）再按同规则比较 ✓；
`step == 0` / `NaN` ⇒ 不执行（用 `UCOMISD` + `JP` 显式排除无序 —— C 的 double 比较对 NaN 恒 false ✓）；
操作数复用 `EMIT_NUM_TO_XMM` 三态取值（int48 → CVTSI2SD / 裸 double → MOVQ / NaN-boxed → bailout）✓
⇒ 对象 / BigInt 一律回退解释器 ✓。

**实测**（探针 E/F，与 §8.97 同一对）

| | 修前 | 修后 |
| --- | --- | --- |
| E（浮点步长，fm `render` 形态）| 174~196 ms（≈NO_JIT 158ms ✗）| **95~111 ms** ✓ |
| F（int 步长对照）| 93~97 ms | 103 ms（噪声内 ✓）|

⇒ **≈1.8~2x** ✓，且与 int 步长**持平** ✓。回归：6 枚探针（含 `probe_floatstep_toplevel` 的
正向/反向/零步长/嵌套四形态）JIT/NOJIT **IDENTICAL** ✓、assert **311/0** ✓、
三真实应用 `sum` 逐字一致、`upf` 全在噪声内（fm 3909 / cc 5547 / 五子棋 2967）✓。

**残余（诚实记录）**：**顶层**浮点步长循环（它自己就是编译区域、不套在别的热循环里）仍然
bailout ✗ —— 序言里的 `-3` 守卫（`x86_64.c` 的 loop prologue）还没放开。本次**尝试过**并
**已回退** ✗：给序言接 SSE 入口后，嵌套形态每轮**少算一轮** ⇒ 探针 E 的 checksum 由
`1800000` 变成 **`1799570`** ✗（静默错值，比慢更坏）⇒ 按「宁可不编，不要猜」回退 ✓。
证据表明**收益与风险是分开的**：2x 完全来自 `FOR_PREP`/`FOR_LOOP` 这两条路径 ✓
（序言改动之前实测已是 98~101ms ✓）。若将来再动序言入口，先查"入口那次增量与首轮体执行"
的计数关系（少一轮的形态指向那里）。

**顺带修掉我自己埋的一处诊断缺陷**：§8.97 给 `step == 0` 用的基址 `-2000 - off` 会与
"非溢出类"的 `-1000 - off` **撞号** ✗（只要 `off >= 1000` 就落进 `<= -2000` 区间 ⇒
被误报成 `FOR_PREP: step == 0` ✗；fm 的 `off=64674` 就撞上了）⇒ 该编码与对应分支**已删除** ✓，
`jit_bailout_reason` 恢复为三条区间（`>=0` / `-999..-1` / `<= -1000`）✓。
教训：**给诊断编码开新区间前，先确认它与既有区间的值域不重叠** ✓。

***

### 8.114 【已修·诊断】`Bailout: … 触发指令=` 对**内联帧**会报出不可能的指令名（如 `OP_NULL`）（2026-09-17）

**症状**（植物大战僵尸 pvz 实测）：
```
Bailout: fn='drawMowers' loop_bc=13 x3 — 非溢出类 @bc_off=151（…，内联深度 2） | 触发指令=OP_NULL
```
`OP_NULL` 是纯压栈指令 ✓，不可能是 bailout 原因 ✗ ⇒ 一看就是**诊断在骗人** ✗。

**根因**：`jit.c` 的"触发指令"还原用 `frame->chunk->code[loop_bc + rel]` ✗ —— 而**内联帧**的
bailout 发生在**被内联函数**的字节码里（另一个 `chunk` ✓），`frame->chunk` 是**调用方**的
⇒ 读到的只是"那个偏移上碰巧是什么字节" ✗。深度 0 的那些（`OP_MODULE_CALL` / `OP_CALL` /
`OP_CALL_NATIVE` ✓）是真值 ✓，所以问题被掩盖了很久 ✗。

**修法**：按偏移里的 `0x10000 * depth` 基址先取 `depth`；`depth > 0` 时如实打印
`<不可还原：内联帧的字节码在 callee chunk>` ✓，不再给一个"看着像答案的错值" ✗。
（彻底的修法需要在 JIT 缓存里存"每个内联深度对应的 chunk" ✓，属待办 ✓。）

**教训（与 §8.97 的撞号同族）**：**诊断代码也在给"事实"下结论** ✓ —— 它同样要守
"要么准确、要么明说不知道" ✗；错的操作码名会把排查引到完全错误的方向 ✓
（本次若不查，很容易去追 `OP_NULL` 的 JIT 支持 ✗，那是死路 ✓）。

***

### 8.115 【已修】§8.106 的"歧义区 ⇒ bailout"守卫判据写错：它等于**所有非负 int48** ⇒ 热循环被大面积拉黑（2026-09-17）

**怎么发现的**（PvZ 的观测驱动 ✓）：60 帧带 `LENO_JIT_DEBUG=1` 跑一遍，callout 日志 72841 行里
**只有 16 条失败**，且全是这一条 ✗：
```
x13 [JIT-CALLOUT-FAIL] native 实参落在歧义区 ⇒ 交解释器（§8.106）
x3  [JIT-CALLOUT-FAIL] native 直调实参落在歧义区 ⇒ 交解释器（§8.106）
```
而同一次运行里 `call_value 委派 VM 重入` **成功 26091 次** ✓、`native 直调成功` **8696 次** ✓
⇒ **缺口不在这里** ✗；21 条 bailout（7 循环 ×3）正是被这 16 次失败反复命中后拉黑造成的 ✓。

**根因**：守卫的判据是 `(raw >> 47) == 0` ✗，它等价于 `0 ≤ raw < 2^47` —— 也就是
**所有非负 int48** ✗（只有 `+0.0` / 正次正规数与它**重叠**，但重叠区被写成了整个区间 ✓）。
于是任何带**非负整数实参**的 native 调用点都会 bail ✗ —— 这类实参在真实代码里遍地都是 ✗
（`T.malloc_array(vcnt)` / 任何 `0`、`1`、`count` ✓）。

**修法**：**按声明形参类型消歧**，不再整体 bail ✓

| 路径 | 做法 |
| --- | --- |
| `jit_callout_module_call_meta` / `jit_callout_module_call` | 取 `meta->param_types[i]`（表内已把 arity 之外填成 `TYPE_ANY` ✓）⇒ 声明为 `float` 而值是 int ⇒ **提升成 double** ✓（`+0.0` 与 `int 0` 数值相同 ⇒ 无损 ✓）；其余按整数 ✓ |
| `native 直调`（`T.malloc()` / `c.free()` 这类已分派路径）| 拿不到声明类型 ⇒ 只删掉整体 bailout ✓，实参按整数解读装入 ✓（值正确 ✓）；**残余**：native 若真关心"这个 0 是 float 还是 int"（`is float` / 字符串化）会有**类型标签**分叉 ✗ —— 该路径上极罕见 ✓，模块方法那条已精确提升 ✓ |

**实测（同口径 `us_per_frame`）**

| | 修前 | 修后 |
| --- | --- | --- |
| PvZ | `21111`，Bailouts **21**（7 循环 ✔ 全黑）| **20295~20460**（≈3.5% ✓），Bailouts **9**（3 循环）|
| fm / cc / 五子棋 | Bailouts 21 / 12 / 3 | **0 / 0 / 0** ✓✓ |

fm 复测 3789 / 3794 / 3971（基线区间 3791~3909 ⇒ 噪声内 ✓ 无回归 ✓）；6 枚探针
（含 `probe_native_arg_float` / 次正规三兄弟 ✓）**IDENTICAL** ✓；assert **311/0** ✓。

**剩余（PvZ）**：`loadAnim` ×2（`OP_CALL_NATIVE`，**加载期** ✓ 不值当 ✓）、
`drawHUD` 的 **泛型 `OP_ADD`** ✓（动态类型算术 —— 下一个真正的目标 ✓）。

**教训**：**"保守的 bail"如果会在普通输入上触发，它就不是保守，而是 bug** ✗。
把一个混合区间的**上界**当成整个区间来判（`0 ≤ raw < 2^47` 里只有两端与浮点重叠 ✗），
代价就是把最常见的一类实参全挡在门外 ✓ ⇒ 立守卫时**必须拿真实语料数一数它触发多少次** ✓
（本次 60 帧就现形 ✓，若只看 mp/cc 这类实参以指针为主的样例，会误以为它无害 ✗）。

***

### 8.116 【已修】§8.103 的 concat 守卫同族缺陷（`"" + (i+1)` 每次迭代都 bail ⇒ drawHUD 循环被拉黑）；并用"来源证明"精确放行（2026-09-17）

**症状**：PvZ `drawHUD` 的 `for` 循环（`loop_bc=216`、`body_size=93`）bailout @ `rel=61`
`触发指令=OP_ADD` ✓ —— 该循环体里唯一相关的表达式是 `r.drawTextCentered(..., "" + (i + 1), ...)` ✓。

**根因**：`OP_ADD` 的 **concat 段**带着 §8.103 的"歧义区 ⇒ bailout"守卫 ✗
（`ops_arith.inc`，判据同样是 `raw>>47 == 0` ⇔ **所有非负 int48** ✗，与 §8.115 同一个写法缺陷 ✓）
⇒ `"" + (i+1)` 的右操作数是小非负整数 ⇒ **每次迭代都 bail** ✗ ⇒ 三次后循环被拉黑 ✓。

**为什么不能像 §8.115 那样直接删**：拼接的歧义**语义上真的不同** ✗ ——
`"" + 0.0` = `"0.0"` 而 `"" + 0` = `"0"` ✓（§8.103 当初就是为这个立的守卫 ✓，探针
`probe_float_param_faithful` 钉着它 ✓）。所以必须**证明**它是整数才能放行 ✓。

**修法：用既有的"来源证明"（§8.41）精确放行右操作数** ✓
- `prev_raw_int48`（`EMIT_INT48_CHECK_TOS` / `MARK_TOS_RAW_INT48` 置位）表示
  **紧邻的上一条指令**产出的 TOS 已被证明是裸 int48 ✓，白名单只收**单路径**指令 ✓
  （通用 `OP_ADD` 明确不在白名单里 ✓ ⇒ 不会误信 ✓）；
- concat 段的**右操作数就是 TOS** ✓ ⇒ `if (!prev_int48)` 时才做歧义检查 ✓；
  **左操作数没有这个保证 ⇒ 原检查原样保留**（不猜 ✓）。
- 于是 `"" + (i+1)`（`i+1` 走 `ADD_INT_IMM` 白名单 ✓）**免检通过** ✓，而
  `"" + p`（p 是 float）仍然会 bail ✓ —— 语义**精确**，不是放宽 ✓。

**顺带删掉第 4 处 §8.106 守卫**（`jit_callout.c` 的 `OP_CALL_NATIVE` 直接调用路径 ✓）——
同 §8.115 的理由（判据等于"所有非负 int48" ✗）。实测它**不是** `loadAnim` 那两条
`OP_CALL_NATIVE` bailout 的原因 ✗（删后仍在 ✓，另因待查 ✓；那是**加载期** ✓ 低优先 ✓）。

**实测**

| | 修前 | 修后 |
| --- | --- | --- |
| PvZ Bailouts | 9（3 循环：drawHUD + loadAnim×2）| **6**（只剩 loadAnim×2 ✓）|
| 拼接语义探针 | — | `probe_float_str_point` / `probe_float_param_faithful` / `probe_type_check_float_zero` **IDENTICAL** ✓ |
| 全套 / 探针门禁 | — | assert **311/0** ✓、**19/19 IDENTICAL** ✓ |

**⚠ 本轮的应用级帧耗时数据不可用（如实记录）**：同口径重跑 PvZ 时 SDL 自身的
`fill` 由 `4507us` 跳到 `5661us`（+25% ✗）⇒ **机器负载变了**，`us_per_frame`
（21352~21536 vs 20295~20725）**不能用来判收益** ✗。本轮可靠的结果是"**覆盖**"：
`drawHUD` 不再被拉黑 ✓；帧耗时待机器安静时重测 ✓。

***

### 8.117 【部分】§8.100 的 `is`/`as` 歧义守卫同族缺陷 —— 加了两条"证明"通道，但**实测未生效**（诚实记录）（2026-09-17）

**目标**：§8.100 那条 `is`/`as` 守卫的判据同样是 `raw>>47 == 0` ⇔ **所有非负 int48** ✗
⇒ 热循环里 `v is int`（v 是非负整数）**每次迭代都 bail** ✗ ⇒ 三次后拉黑 ✓。

**改动**（`ops_callout.inc`）：在歧义检查之前先试着**证明** TOS 是裸 int48，证明了就不猜 ✓。
两条证明都取自**既有**判据（不新增假设 ✓）：
- (a) `prev_int48`：紧邻的上一条指令在白名单里（§8.41）⇒ 结果必是裸 int48 ✓；
- (b) `tos_from_si` + RBX 位图 bit=0：与 `EMIT_RAW_TO_VALUE`（装箱）用的是**同一条**判据 ✓
  （要么一起信、要么一起不信 ✓）。

**⚠ 实测结果：数值正确但**没有**达到预期效果** ✗（不掩饰 ✓）
- 新增探针 `jit_probes/probe_is_int_loop.leno`（`is int` 与 `is float` 两形态 ✓）：
  数值 **IDENTICAL** ✓（`isInt=50000 isFloat=50000` ✓）**但两个循环仍各 bail 3 次** ✗（`Bailouts: 6` ✗）。
- 查证：`tos_from_si` **只由 `OP_GET_LOCAL` 置位** ✓（`ops_stack.inc:43` ✓）⇒ (b) 的适用范围正是
  "操作数是局部量"这一最常见形态 ✓，而它依赖的 RBX 位图**并不能**给出"该槽是 int" ✗ ——
  它标的是"**非 int**（float/obj/bool/null ✓）"，**反向不成立** ✗（§8.104 早已记录 ✓）
  ⇒ 对本探针的 `v`，位图 bit 显然是 1 ✗ ⇒ 跳过条件不成立 ⇒ 仍走 bail ✓。
- (a) 只在"操作数由白名单算术**直接**产出"时生效 ✓（例如 `(i + 1) is int` ✓——真实但窄的形态 ✓）。

**⇒ 结论（重要）**：这条守卫要**真正**放开，前提是**可靠的槽级类型信息** ✗ ——
也就是 §8.107 / §8.108 那次未完成的工作（位图**忠实维护** / 粘性来源跟踪 ✗）。
本次改动**保留**（精确 ✓、与既有判据一致 ✓、零风险 ✓），但**不宣称任何收益** ✓，
并把"槽级类型信息"重新标为这条线的**前置项** ✓（§8.100 末尾的"待决"在此升级为"卡点" ✗）。

**同族清点（本轮盘出来）**：判据 `raw>>47 == 0` 的守卫共 **5 处** ✗ ——
§8.115 三处（native 实参，已修/已换判据 ✓）、§8.116 一处（concat，已用来源证明精确放行 ✓）、
本处一处（`is`/`as`，证明通道未生效 ✗）；另有 §8.105 的**字面量**两处（数组/字典字面量的
元素由算式产出时会 bail ✗）—— 那两处**没有元素级来源信息** ✗，属同一卡点 ✓，先不动 ✓。

**回归**：assert **311/0** ✓；fm / cc / 五子棋 **Bailouts 0/0/0** ✓、PvZ **6**（与 §8.116 后一致 ✓）✓；
四应用 `sum` 与之前逐字相同 ✓；本轮涉及的探针全 **IDENTICAL** ✓（含 `probe_is_int_ambiguous` ✓、
`probe_type_check_float_zero` ✓、`probe_float_str_point` ✓）。

***

### 8.118 §8.113 + §8.115 + §8.116 的**累计实测**（干净口径：`us_per_frame` 与 `fill` 同读）（2026-09-17）

同一台机器、300 帧、无头（`SDL_VIDEODRIVER=dummy`），每项 3 轮；**同时记录 SDL 自己的
`fill` 作为负载指标** ✓（上一轮就是因为它从 4507 跳到 5661 才判数据不可用 ✗）。
"引擎时间 = `us_per_frame` − `fill`"用于跨负载对比 ✓。

| 应用 | 现在（3 轮）| 基线 | 引擎时间 | Bailouts |
| --- | --- | --- | --- | --- |
| **PvZ** | upf 21906 / 21931 / 22077（fill 5661）| 21111（fill 4507）| **16245 / 16270 / 16416** vs 16604 | **6**（原 21）|
| fm | 3987 / 3925 / 3852（fill 4500）| 3791~4017 | 持平 | **0**（原 21）|
| cc | 5591 / 5554 / 5573（fill 1800）| 5490~5548 | 噪声内 | **0**（原 12）|
| 五子棋 | 3009 / 3015 / 3018（fill 900）| 2958~2995 | 噪声内 | **0**（原 3）|

**结论（重要，且对后续排序有指导意义）**

1. **覆盖面**：拉黑的循环 **7 → 2**（PvZ 剩的两条是 `loadAnim`，**加载期** ✓）；
   fm / cc / 五子棋 全部 **0** ✓ —— 这一项是**干净**的 ✓、可复验 ✓。
2. **帧耗时**：只有 PvZ 约 **1~2%** ✓，其余三个在噪声内 ✓ ⇒ **那些被拉黑的循环并不是
   帧时间的瓶颈** ✗。因此"继续把循环覆盖率做上去"对**应用帧耗时**的期望收益**很小** ✗ ——
   这一点值得记下来，避免把力气继续投在边际很小的方向 ✓。
3. **时间花在哪**：PvZ 每帧 `us_per_frame≈21ms`，其中 SDL 自己的 `fill` 占 **5.6ms** ✓，
   剩下 **≈16ms 是 Leno 侧** ✗ ⇒ 下一步应当**先做一次分解** ✗（哪几个函数/循环吃掉这 16ms ✓），
   而不是继续按 bailout 列表挑 ✓。可用的抓手：`Compiled: 26 / FuncCompiled: 15 / FuncExecuted: 919`
   ⇒ 每帧只有约 3 次函数级 JIT 执行 ✓ ⇒ **绝大多数 per-frame 代码仍在解释器里** ✗
   ⇒ 真正的方向多半是"**为什么 per-frame 的绘制函数没被函数级 JIT 接住**" ✓。

***

### 8.119 判定实验：应用帧耗时里的"Leno 侧"到底值多少 —— 结论是**引擎不是瓶颈**（2026-09-17）

§8.118 指出 PvZ 每帧 ≈21ms 里 SDL `fill` 只占 5.6ms、剩 ≈16ms 记在 Leno 头上 ✗。
本轮用 `jit_probes/probe_frame_cost_split.leno` 把循环内部的三种边界成本**直接量出来** ✓
（每种 2e6 轮，JIT 与 NO_JIT 各一遍 ✓）：

| 形态（2e6 轮）| **JIT** | NO_JIT | 边界成本（JIT）|
| --- | --- | --- | --- |
| 纯算术循环 | **2.55 ns/轮** | 36.2 ns/轮 | —（JIT 快 **14x** ✓）|
| 循环里**每次调 native** | 35.4 ns/轮 | 61.2 ns/轮 | **≈32.9 ns/次** ✓ |
| 循环里调 **Leno 小函数** | 5.16 ns/轮 | 46.0 ns/轮 | **≈2.6 ns/次**（被内联 ✓✓）|

（交叉验证：NO_JIT 的 36.2 ns/轮与 §8.97 探针 E/F 的 2.6e6 轮 ≈100ms ⇒ ≈38 ns/轮 一致 ✓。）

**归因**（代入 PvZ 的实测频次：native ≈145 次/帧 ✓）

- native 调用边界：145 × 32.9 ns ≈ **4.8 µs/帧** ✗ —— 对 16ms 完全可忽略 ✓；
- Leno 函数调用边界：≈2.6 ns/次 ✓（内联机械把它压到与纯循环同量级 ✓）；
- 若那 16ms 真由**我们**的代码产生，按 JIT 循环 2.55 ns/轮 反推需要 **≈640 万轮/帧** ✗
  —— 对一个 2D 游戏完全不成立 ✗。

**结论（这决定后续排序 ✓）**：那 ≈16ms 基本发生在 **SDL 内部**（软件渲染的纹理/几何/上传 ✓），
只是记在"调用方 = Leno"头上 ✓ ⇒

1. **继续优化 JIT 对这类应用的帧耗时边际≈0** ✗（与 §8.118 的 1~2% 吻合 ✓）——
   引擎侧的工作应当改用**别的口径**衡量 ✓：JIT 执行占比、bailout/拉黑数、正确性、编译耗时、
   以及**引擎受限**的基准（`examples/性能测试/**` 那类 ✓），不要再用游戏帧耗时当 KPI ✓。
2. 顺带确认：**内联机械表现很好** ✓（2.6 ns/次的调用边界 ≈ 纯循环一轮 ✓），
   native callout 的 32.9 ns 也属正常量级 ✓ —— 这两处都不必再优化 ✓。

***

### 8.120 换口径后发现的两个异常 ⇒ 根因是**JIT 的调用边界比解释器派发还贵**（2026-09-17）

**怎么找到的**（按 §8.119 的结论换成"引擎受限"口径 ✓）：`examples/性能测试/**` 一批微基准同时跑
JIT / NO_JIT ✓，**提速比最小**的就是引擎侧剩下的最大缺口 ✓：

| 基准 | JIT | NO_JIT | 判读 |
| --- | --- | --- | --- |
| `While vs For`（1 亿空循环）| 70.9ms | 542.8ms | 7.7x ✓ |
| `调用开销微基准` | 22.2 / 28.4ms | 460 / 456ms | ≈17x ✓ |
| `参数扩展性`（1/7 参）| 24.2 / 50.0ms | 463 / 543ms | ≈11~19x ✓ |
| `光线追踪` Phase A/B | 35.4 / 107.9ms | 210 / 749ms | ≈6~7x ✓ |
| ⚠ `函数调用数学计算` | **4587ms** | **4831ms** | **仅 5%** ✗ |
| ⚠ `内联缓存` 测试1（`arr.len()` ×100 万）| **17.87ms** | **13.45ms** | **JIT 更慢** ✗✗（负优化）|

**根因（用 `probe_callout_kinds` 分种类量出来 ✓）**

| 调用种类（2e6 轮）| **JIT** | NO_JIT | 判读 |
| --- | --- | --- | --- |
| 纯循环（参照）| 2.40 ns/轮 | 36.4 | JIT **15x** ✓ |
| `maths.sqrt(i)`（模块方法）| **12.14 ns** | **4.75 ns** | JIT 贵 **2.6x** ✗ |
| `arr.len()`（`OP_CALL` 实例方法）| **20.01 ns** | **4.73 ns** | JIT 贵 **4.2x** ✗✗ |
| `times.ms()`（模块方法、无参）| 31.74 ns | 20.98 ns | JIT 贵 1.5x ✗（本体即贵 ✓）|

**结构性结论**：解释器**本来就在 C 里** ✓ —— 它的"派发"就是一次直接 C 调用 ✓；而从 JIT 机器码里
每次 callout 都要付：保存/恢复 JIT 状态（RSP/RCX/R9 → tmp 槽 ✓）+ ABI 对齐 + **失败标志检查** ✗
（`EMIT_CALLOUT_BEGIN/END` ✓，≈8~16ns ✓）。于是：

- **循环开销主导**（纯算术、1 亿空循环、参数扩展）⇒ JIT 大胜（6~19x ✓）；
- **"每轮一次调用"主导**（`内联缓存`、`函数调用数学计算`）⇒ JIT 的边界税超过它省下的循环开销 ✗
  ⇒ 轻则只有 5% ✓、重则**负优化** ✗。

**可行性判断（据实）**：边界里能安全砍掉的是"**没有失败通道**的 callout 的失败检查" ✓
（`jit_callout_string_add` / `type_check` / `is_null` 都不置 `failed` ✓，§8.64/§8.65 已注明 ✓）——
省掉一次 `movabs`(10 字节常量) + load + test + branch + clear ✓；但**可失败**的调用（模块方法 /
native ✓ —— 恰恰是两个异常的主角 ✗）**必须保留**检查 ✓ ⇒ 边界改进对这两个异常的帮助**有限** ✗。
更根本的选项是"**调用密度高的循环不交给 JIT**"✗ —— 但那是策略启发式 ✗（脆 ✓），且
`函数调用数学计算` 实测 JIT 仍**更好** ✓（4587 < 4831 ✓）⇒ 交叉点在"每轮 1~2 次调用"附近 ✗
⇒ **不建议**按密度做策略 ✓。本项**先记为"已知结构性权衡"** ✓，不建议为它单独开工 ✓
（真实应用里更常见的是"每次调用都摊到几十轮循环上" ✓，此时 JIT 依然净胜 ✓）。

***

### 8.121 覆盖面普查（`jit_census.ps1`）—— 运行期 bailout 三应用**全部为 0**；静态侧缺口清单（2026-09-17）

跑 `jit_probes/jit_census.ps1`（300 帧无头 ✓），本轮这一大段（§8.113 / §8.115 / §8.116）的
净效果在这一行里最能看出来 ✓：

| 应用 | 运行期 | 扫描期拒绝（静态缺口）| upf（同口径）|
| --- | --- | --- | --- |
| fm | **Bailouts: 0** ✓（原 21）| 31 内联超上限 / 22 函数体含循环（12+10）|  `/ 12+10 函数体含循环` / 3 `OP_GET_MODULE_FUNC` / 3 循环体含 return | 4019 |  
| cc | **Bailouts: 0** ✓（原 12）| 20 内联超上限 / 13 函数体含循环（7+6）/ 3 `OP_GET_MODULE_FUNC` / 2 循环体含 return | 5542 |  
| 五子棋 | **Bailouts: 0** ✓（原 3）| 9 内联超上限 / 3 函数体含循环 / 2 未类型化 `OP_CALL_GLOBAL_FUNC`（§8.96 已裁决不做）/ 1 `OP_GET_MODULE_FUNC` | 2949 |  

**读法**：`[runtime bailout] (none)` ✓ = 稳态运行中**没有任何循环被拉黑** ✓ —— 这是本轮三修的
直接结果 ✓（也是 §8.118 说"帧耗时收益小"的另一面：**覆盖面**已经干净了 ✓，瓶颈在别处 ✓）。

**静态缺口清单（按出现次数排序，供后续立项）**

1. **函数体含循环（func_mode 不支持）** ✗ —— fm 22 / cc 13 / 五子棋 3 ✓ **条目最多** ✓：
   函数级 JIT 拒绝"含循环的函数体" ✓（`OP_FOR_PREP` / `OP_LOOP` ✗）。热循环本身仍由**循环级**
   JIT 接住 ✓，所以这条的代价是**函数外壳那部分**（进入/退出、循环外的代码 ✓）留在解释器 ✗。
2. **内联未尝试：方法体字节码长度超上限** ✗ —— fm 31 / cc 20 / 五子棋 9 ✓ **第二多** ✓：
   纯**参数**问题（内联大小上限 ✓）⇒ 最便宜的实验是先量"抬高上限"的收益与编译耗时代价 ✓
   （风险：代码膨胀 ✓）。
3. `OP_GET_MODULE_FUNC`（§8.95 未放行 ✓）—— 三应用各 3 处 ✓。
4. 循环体含可达 return ✗ —— fm 3 / cc 2 / 五子棋 2 ✓。
5. 未类型化 `OP_CALL_GLOBAL_FUNC`（五子棋 2 处 ✓）—— **§8.96 已裁决不做** ✓（需 callee 身份守卫
   + 参数提升两套机械，投入产出比不成立 ✓），此处仅作计数保留 ✓。

⇒ **下一项的候选**：1 是"条目最多"✓ 但属**结构性**（要做"函数体含循环"的支持 ✗）；
2 是"第二多且最便宜"✓（先量再定 ✓）。两者都不影响正确性 ✓，可按收益/风险自行排序 ✓。

***

### 8.122 【已否掉】内联上限 256 → 1024：用**正确口径**重测，结论仍是"不要做"（但理由与 §8.95 不同）（2026-09-17）

**为什么值得重测** ✓：§8.121 的 GAPS 普查显示"**内联未尝试：方法体字节码长度超上限**"是
**第一名**（fm 31 / cc 20 / 五子棋 9 ✓），而 `jit_scan.c` 的注释明确留了叮嘱：
"下次想调它，先看 `LENO_JIT_GAPS` 里'长度超上限'是不是第一名" ⇒ **条件满足** ✓。

**但 §8.95 当年的 A/B 用的是帧耗时** ✗ —— 按 §8.118/§8.119 的结论，那正是**测不出引擎收益**的
口径 ✗ ⇒ 当年的"无收益"判断**证据不牢** ✓，所以这次换成**能测的口径**：直接数**调用边界次数** ✓
（`LENO_JIT_DEBUG=1` 下的 `[JIT-CALLOUT]` 行 ✓，60 帧 ✓）。

| 内联上限 | fm callout 行 | cc callout 行 | GAPS 里"长度超上限" |
| --- | --- | --- | --- |
| **256**（现状 ✓）| **9844**（native 4922 + 委派 4922）| **506** | fm 31 / cc 20 / 五子棋 9 |
| 1024 | 9850（4925 + 4925）| 506 | fm **2** / cc **1** / 五子棋 **0** |

**读法（这就是新理由 ✓）**：上限抬高**确实**让那些方法体成功内联了 ✓（GAPS 里从 31/20/9 掉到
2/1/0 ✓）—— 但**热路径上的调用边界只少了 6 次 / 60 帧** ✗（≈0.1 次/帧 ✗），cc **一次都没少** ✗。
⇒ 那些"超上限"的方法体**不在热调用路径上** ✓（内联了也不省边界 ✓），
而 §8.95 当年看到的"新 callee 被别的门挡住"只是**次要**原因 ✓。

**裁决：还原为 256** ✓（工作树已还原 ✓，`git diff` 空 ✓）——与 §8.95 结论一致 ✓，
但理由改写为"**被内联者不在热路径上**" ✓（下次再有人看 GAPS 第一名想动手时，先看这条 ✓）。

**方法论收获（值得记）**：同一个改动，**换口径会换结论** ✓ ——
§8.95 用帧耗时（引擎盲 ✗）⇒ 只能得出"测不出" ✗；本次用**调用边界计数**（引擎敏感 ✓）
⇒ 能看出"确实内联了、但省不到边界" ✓ ⇒ 结论更硬 ✓。
**立 KPI 时要先确认这个 KPI 对该改动敏感** ✓（§8.118/§8.119 的同一教训 ✓）。

***

### 8.123 【已修】大整数三条缺陷：`>>` 段错误 / `<<` 静默 48 位截断 / 字面量与计算值语义分裂（2026-09-17，用户报告）

**共同根因**：**溢出检测依赖 UB** ✗ + **压缩回 int 后未按实际类型分派** ✗ + **常量折叠用 double 且无范围守卫** ✗。

#### bug1（段错误）—— 两个独立缺陷叠加，已插桩定位到确切一行

1. `0x80000000 * 4294967296`（2^31 × 2^32 = 2^63 ✗ 超出 int64 ✗）走 `OP_MUL_INT`，
   而它的溢出检测写作 `int64_t result = a * b; if (result / a != b)` ✗ ——
   **溢出乘法本身就是 UB** ✗，编译器有权按"不会溢出"假设把那条判等**优化掉** ⇒
   直接走 `val_bigint_from_int64(-2^63)` ⇒ 得到一个 **负** 的 2^63 ✗（= bug3 ✓）。
2. 这个负 bigint 再 `>> 60` ⇒ `bigint_shr` 的**负数 round_up 分支** ✗：
   `bigint_add(result, 1)`（7+1=8 ✓）内部会 `bigint_compact_to_int` ⇒ **落回 int48 时返回 int** ✗ ⇒
   紧接着 `bigint_neg(val_as_bigint(added))` 里的 `val_as_bigint(int)` = **NULL** ✗ ⇒
   `a->limb_count` 解引用空指针 ⇒ **0xC0000005** ✓✓（插桩 `LENO_DBG_SHR=1` 的最后一行正是
   `[SHR] round_up: added ok, bigint_neg` ✓，其后即崩 ✓）。

**修法**：溢出检测改用 `LENO_MUL_OVERFLOW`（`__builtin_mul_overflow`，无该内建时用纯整数比较 ✓，
**不做溢出乘法** ✓）；`bigint_shr` 的 round_up 分支**按实际类型分派**（int ⇒ 整数域取负 ✓，
bigint ⇒ `bigint_neg` ✓，其它 ⇒ 返回 null 不猜 ✓）。插桩保留（`LENO_DBG_SHR=1` 才输出 ✓）。

#### bug2（静默截断）—— 不在"字面量类型派生"，而在**移位结果没有溢出提升**

`OP_SHL_IMM` 的 int 快路径原是 `val_int(a << imm)` ✗ —— `val_int` 会把结果**截断到 int48** ✗：
`0x123456789ab << 8` 是 **49 位** ✗ ⇒ 最高位被丢（实测 `000023456789ab00` ✗ vs 期望 `000123456789ab00` ✓）；
而 >48 位字面量的同形态却正确（走 bigint 路径 ✓）⇒ 两条路径语义不一致 ✓。
**修法**：先按移位前的值判断是否装得下 int48（`_a >= 0 ? _a <= INT48_MAX >> imm : _a >= INT48_MIN >> imm` ✓），
装不下就提升 bigint ✓（与 ADD/SUB/MUL 的溢出语义一致 ✓）。

#### bug3（语义分裂）—— **运行时**与**编译期**两条路径都要修

- 运行时：`OP_MUL_INT` / `OP_MUL_INT_IMM` / `PUSH_SAFE_INT64_MUL(_FAST)` 三处全是那条 UB 检查 ✗ ⇒ 全换 ✓；
- 编译期：`optimize.c` 的 `fold_binary` 用 **double** 折叠且**没有范围守卫** ✗ ⇒
  `0x80000000 * 4294967296` 折出 2^63（double 只能精确表示 ≤ 2^53 的整数 ✗）⇒ 回落 int64 再次溢出 ⇒ **-2^63** ✗。
  修法沿用既有策略（与"移位量 ≥ 32 就不折叠、交给 VM 的 BigInt 路径"同一句话 ✓）：**不精确就不折** ✓
  —— 折叠只是优化，跳过它语义不变 ✓。

**验证**（`_jit_bugs/` ✓，全部 `LENO_NO_JIT=1` 与 JIT 两遍 ✓）

| 文件 | 修前 | 修后 |
| --- | --- | --- |
| `bug1_segfault_shift.leno` | **exit=0xC0000005** ✗（L5 处）| 跑完 ✓ `L5(2^63+1) = 8000000000000001` ✓ |
| `bug1_min_split.leno`（本次新增的崩点切分）| s4 崩 ✗ | `s4 c1>>60 = -8` ✓ / `s5 = -2147483648` ✓ |
| `bug2_shl48.leno` | `a_shl8 = 000023456789ab00` ✗ | **`= 000123456789ab00`** ✓（与 `a*256` 一致 ✓）|
| `bug3_control_toplevel.leno` | `v = -9223372036854775808` ✗ | **`v = 9223372036854775808`** ✓ / `q1 = 2147483648` ✓ |
| `bug3_min_literals.leno`（本次新增的字面量判别）| `L_e` 负 ✗ | `L_e = 9223372036854775808` ✓（且 `L_h`（变量）也一致 ✓）|

assert **311/0** ✓。

**残余（已确认，列为下一步 ✓）**

1. **JIT 的 `OP_SHR` 仍 bailout** ✓（用户第 ④ 点 ✓）：`hex64` 的 `v >> (60 - i*4)` 是**动态移位量** ✗ ⇒
   JIT 回退解释器 ✓ —— **正确但慢** ✓（`Bailouts: 3` / `Compiled: 1` ✓）。修 VM 后 bailout 未消失 ✓
   （两者本独立 ✓），属**性能**项 ✓。
2. **新发现：掩码语义** ✗ —— `(0x80000000 * 4294967296) & 0xffffffffffffffff` 打印
   **-9223372036854775808** ✗（按任意精度语义应为 **+2^63** ✓）⇒ 疑似 `bigint_and` 的符号/补码处理 ✗
   （与用户 bug3 末尾那条"`& 0xffffffff` 掩码语义"线索吻合 ✓）。**下一项就查它** ✓。

***

### 8.124 【已修】大整数位运算的**符号与补码**：`&`/`|`/`^` 两个缺陷叠加（2026-09-17）

**症状**（用户 bug3 末尾那条"`& 0xffffffff` 掩码语义也可能有 32 位带符号中间值的问题"✓，实测确认 ✓）

```
(0x80000000 * 4294967296) & 0xffffffffffffffff   ⇒ -9223372036854775808  ✗（应 +2^63 = 9223372036854775808 ✓）
-1 & 0xffffffffffffffff                          ⇒ 4294967295            ✗（应 +2^64-1 = 18446744073709551615 ✓）
-1 | 0xffffffffffffffff                          ⇒ -79228162495817593519834398721 ✗（应 -1 ✓）
-1 ^ 0xffffffffffffffff                          ⇒ -79228162495817593524129366016 ✗（应 -2^64 ✓）
```
判别的关键（本轮实测）：**`+` / `-` / `*` 全对** ✓、`mask & mask` ✓、`mask | 0` ✓、`mask >> 32` ✓
⇒ 只有**位运算**坏 ✗，且只在**含负操作数 / 结果幅值用到最高位**时坏 ✗。

**根因（两个缺陷叠加，缺一不可修）**

1. **符号判据错** ✗（`bigint_and` / `bigint_or` / `bigint_xor` 三处同样写法）：
   ```c
   int result_negative = (result_limbs[max_limbs - 1] >> (BASE_BITS - 1)) & 1;  // ✗ 看结果最高位
   ```
   这只在**定宽**下成立 ✗ —— 无限精度补码里"幅值 2^63"是**正数** ✓，可它的最高位就是 1 ✗
   ⇒ 被误判成负数 ✓。真判据是**操作数符号** ✓（与 Python 一致 ✓）：
   `and` ⇒ 两负才负 ✓；`or` ⇒ 有负即负 ✓；`xor` ⇒ 恰一负才负 ✓。
2. **`bigint_to_twos_complement_array` 把"幅值"做了符号扩展** ✗：
   ```c
   uint32_t val = get_limb_with_sign_ext(bigint, i);   // ✗ 超出 limb_count 的位给 0xffffffff
   uint64_t sum = (uint64_t)(~val) + carry;            //    取反后变 0 ⇒ 高位补码成了全 0 ✗
   ```
   对 `-1` 求 3-limb 补码本应**全 1** ✓，却得到 `[0xffffffff, 0, 0]` ✗ ⇒ 与掩码 AND 后只剩低 32 位 ✗
   ⇒ 这正是 `-1 & 0xffffffffffffffff = 4294967295` ✗ 的来处 ✓（用户"32 位带符号中间值"的直觉命中 ✓）。
   修法：**幅值不足的 limb 一律补 0** ✓（正数零扩展 ✓、负数零扩展后再"取反加 1" ✓），符号只在这一步体现 ✓。

**顺带把工作宽度加宽一格** ✓：`work_limbs = max_limbs + 1` —— 补码运算要靠符号扩展补齐高位 ✓，
而 **XOR 是三者里唯一会放大幅值的** ✓：`-1 ^ (2^64-1)` = `-2^64` ✗ 需要 **65 位** ✓ ⇒ 只用
`max_limbs` 会算成 0 ✗。

**验证**（新增门禁探针 `jit_probes/probe_bitwise_bigint_sign.leno` ✓，8 行真值表**手算核对过** ✓）

| 表达式 | 期望（= Python 语义 ✓）| 修前 | 修后 |
| --- | --- | --- | --- |
| `a & mask`（a = +2^63）| `9223372036854775808` | `-9223372036854775808` ✗ | ✓ |
| `a \| 0` | `9223372036854775808` | ✓ | ✓ |
| `-1 & mask` | `18446744073709551615` | `4294967295` ✗ | ✓ |
| `-1 \| 0` | `-1` | 巨大负数 ✗ | ✓ |
| `-1 ^ mask` | `-18446744073709551616` | 巨大负数 ✗ | ✓ |
| `-2 & -3` | `-4` | — | ✓ |
| `-1 & 5` | `5` | — | ✓ |
| `-6 ^ 3` | `-7` | — | ✓ |

JIT 与 `LENO_NO_JIT` **逐字一致** ✓；用户那份 `bug4_control_masked.leno` 现为
`calcm=9223372036854775808 / q=8` ✓；§8.123 的三条 bug 文件全部保持正确 ✓（`bug1` 的
`s6 c1 & mask = 9223372036854775809` = 2^63+1 ✓ 也对 ✓）；assert **311 passed / 0 failed** ✓。

**方法论**：这次没有直接改"符号判据"就收工 ✗ —— 先做**切分探针**（`_jit_bugs/diag_mask_split.leno`
与 `diag_promote_neg.leno` ✓）把范围从"位运算全坏"缩到"含负操作数才坏" ✓，再用 `+`/`*` 反证
"提升路径是好的" ✓，才定位到**补码转换**这个真正的一环 ✓。两个缺陷叠加时，
只修一个会让测试"看起来好了一半" ✗ —— 这也是为什么真值表要**一行一行手算** ✓。

***

### 8.125 【已修】JIT 的"**静态 int ⇒ int48**"假设（三处叠加）—— sha512 在 JIT 下全错（2026-09-18）

**症状**：`examples/crypto/sha512.leno` 在 JIT 下**三个标准向量全 FAIL** ✗，`LENO_NO_JIT=1` 全 PASS ✓。

**确定性最小复现**（新件 `_jit_bugs/diag_add64_many.leno` ✓）：同一个
`add64(0x472a31970310840c, 0x0400200000000008)` 连续调用 100 次：

```
i = 0..48   → 4b2a519703108414  ✓
i = 49..99  → 000001b5ea3cec80  ✗（裸堆指针；且**恒定**⇒同一对象地址）
```

**i = 49 恰好是第 50 次调用** ✓ = `jit_priv.h: JIT_FUNC_HOT_THRESHOLD 50` ✓ —— **函数级 JIT
一编译出来就坏** ✓。关掉内联（`LENO_JIT_NOINLINE`）/ 关 GC / 强制 GC / 关循环 JIT：**全都一样** ✗
⇒ 与内联、GC、循环 JIT 无关 ✓。

**根因（三处叠加，各自独立、都属"静态 int ⇒ int48"这一族 ✗）**

| # | 落点 | 缺陷 | 影响 |
| --- | --- | --- | --- |
| ① | **序言 §8.48 类型化形参快路径**（`x86_64.c`，**主因**）| 对 `param_types[slot]==TYPE_INT` 的形参**无条件** `<<16 >>16` 取"int48 载荷" ✗ —— 可 **bigint 的静态类型同样是 `int`**（值是 `0xFFFC|对象指针`）⇒ **标签被削掉、只剩裸指针** ✗ | 编译后的 `add64(int a, int b)` 拿到裸指针，其后全在指针上算 ✗ |
| ② | `OP_SHL_IMM` / `OP_SHR_IMM` / `OP_USHR_IMM` + §8.44/§8.46 **两处折叠路径**（`ops_icmp.inc` / `ops_stack.inc`）| 只靠"扫描器保证操作数是 int48"省掉了检查 ✗（**通用** `OP_SHL/SHR/USHR` 一直**有**这步 ✗，只有 IMM/折叠形态漏了 ✗）| 表现为"**只有常量移位量坏、动态移位量正常**" ✗ |
| ③ | `OP_BITAND` / `OP_BITOR` / `OP_BITXOR` / `OP_BITNOT`（`ops_arith.inc`）| 直接做裸 `and/or/xor/not`，**只检查结果**是不是 int48 ✗ —— NaN-boxed 操作数的**标签会被按位与掉** ⇒ 得到一个"看起来合法的小整数" ✓ ⇒ 结果检查**通过** ✗ | **静默错值**（大整数被 and 成裸指针）✗ |

**修法**

1. **①：停用该快路径** ✓（一律走通用路径：按 tag 分流 —— int48 取载荷 ✓ / float·对象·null·bool
   原样存 + 置 RBX 位图 ✓）。⚠ **诚实记录**：曾在该路径里加过 `tag != 0xFFFB ⇒ jmp 通用路径`，
   但实测**未生效** ✗（行为与直接 `&& 0` 不同 ✗）⇒ 先停用，**不把未验证的写法留在树里** ✓。
2. **②**：三个 IMM 移位 + 两处折叠 + **IMM 比较 / `OP_*_INT` 比较** 全部补
   `EMIT_INT48_CHECK(.…)`：**不合格即 bailout** ✓（与通用路径对齐 ✓）。
3. **③**：四个位运算补**两个操作数**的运行期确认 ✓（`EMIT_INT48_CHECK_REG(RDX)` + `EMIT_INT48_CHECK`
   / 位非单操作数 ✓）。
...
| 验收件 | 结果 |
| --- | --- |
| `_jit_bugs/diag_add64_many.leno`（i≥49）| **全对** ✓（修前全坏 ✗）|
| `examples/crypto/sha512.leno` | **3/3 PASS** ✓（修前 3/3 FAIL ✗），且与 `LENO_NO_JIT` 逐字一致 ✓ |
| sha256 / hmac_sha256 / pbkdf2 | **3/3 / 3/3 / 7/7 PASS** ✓、`FAIL=0` ✓ |
| 新增门禁探针 `probe_bigint_const_ops.leno` | JIT == NO_JIT ✓（bigint × **常量**运算符 ✓：`>>`/`<<`/`&`/`\|`/`^` + 比较 ✓）|
| 新增门禁探针 `probe_bitwise_bigint_sign.leno`（§8.124 那枚）| 仍全绿 ✓ |
| 抽样 7 枚探针（含 `probe_arr_add_loop` / `probe_native_arg_float` / `probe_tiny_make_where`）| 全 **IDENTICAL** ✓ |
| assert 全套 | **311 passed / 0 failed** ✓ |

**代价（诚实测量）** ✗：`jit_probes/nlev_f_intstep_loop.leno`（int 步长热循环）**93~97ms → 104~122ms**（≈ **+10%** ✗）；
float 步长对照 `nlev_e_floatstep_loop` 98~101 → 102~112ms（噪声内 ✓）。约 +10% 来自
①（每个形参多 4 条指令 ✓）与 ②③（位运算/比较/IMM 各多 2~4 条 ✓）两部分 ✓。

**后续可回收（已列待办）**：
- ① 用**能验证的**运行期 tag 确认恢复快路径（下次直接 `LENO_JIT_DUMP=1` 反汇编所生成的那几条指令
  对照 ✓，不要靠读代码猜 ✗）；
- ②③ 用 §8.41 的 `prev_raw_int48`（"上一条指令已证明是裸 int48" ✓）**跳过冗余确认** ✓ ——
  §8.116 在 concat 上已有同款先例 ✓。

**④（写回归测试时**当场**抓出来的第五处 ✗）融合比较跳转 `OP_CMPJMP_LL_INT` / `_LG_INT` / `_LI_INT`**

`vm/vminc/op_jump.inc` 与 `jit/backend/x86_inc/ops_jump.inc` 都**盲用** `val_as_int(...)` 比整数 ✗
（JIT 侧还额外 `shl/sar 16` 取"int48 载荷" ✗）⇒ **两个"值相等但对象不同"的 bigint 被按位型/指针
比成"不相等"** ✗。症状极具辨识度：**同一次迭代里 `(a == b)` 为真而 `if (a != b)` 也走了真分支** ✗✓
（因为 `==` 在表达式里走通用 `OP_EQ` ✓，而 `if` 条件走了融合路径 ✗）。
- **VM 侧修法**：`val_is_int(va) && val_is_int(vb)` 才走原 int64 快路径 ✓，否则交
  `compare_values()` ✓（`vm.c:145`，支持 BigInt ✓，与解释器 OP_EQ/OP_LT… 的数值语义一致 ✓）。
- **JIT 侧修法**：三个 case 各补操作数的 `EMIT_INT48_CHECK` / `_REG` ✓ —— 不合格即 bailout ✓，
  由（已修好的）解释器融合指令接手 ✓。
- 这一条**不是** sha512 的成因 ✗（sha512 是 ①②③ ✓），但它同样是"静态 int ⇒ int48"这一族 ✗，
  而且**正是新回归测试在编写过程中先撞上的** ✓ —— 说明"**绝对期望值断言**"比"JIT vs NO_JIT 对比"
  更早能抓到这类 bug ✓（两模式**都**错 ✗ ⇒ 对比法看不出 ✗）。

**回归套件（2026-09-18 新增 ✓）**：`assert/test_jit_bigint_arith.leno`（套件自动发现 `test_*.leno` ✓）。
覆盖：①乘法/常量折叠（`0x80000000*4294967296` ✓）、②位运算符号与补码真值表 ✓、
③bigint × **常量**运算符（IMM/折叠路径 ✓）、④**超过 50 次**调用的 `add64(int,int)` ✓
（少了这个次数就踩不到"函数级 JIT 生效后"✗）与 3 元 Σ（`xor64(rot…,rot…)`）形态 ✓。
断言一律用**绝对期望值**（`hex64` 字符串 = 手算/已实测输出 ✓）⇒ JIT 与 `LENO_NO_JIT=1`
**两种模式都必须 PASS** ✓（实测两种模式都 exit=0 ✓）。

**方法论（这次最值钱的一条）**：整条链是**逐步切分**出来的 ✓ ——
sha512 全红 ✗ → 结构同形复现 ✓ → **打印实际参与运算的变量**（上一版打印的是"重算一遍的值"，掩盖了问题 ✗）✓
→ `add64` 门内实参正确 ✓ → 返回值坏 ✗ → **阈值 49 = 50 次调用** ✓ → 关掉类型化形参快路径 ⇒ **全绿** ✓✓。
...
### 8.127 【已修】`OP_ARRAY` 元素类型污染：用"来源证明"替代值域判据（`[0.0]` / `[z]` 的 float 被贴成 int）（2026-09-18）

**背景**：§8.126 撤销 §8.105 守卫后留下的残余 —— JIT 区域内的数组字面量，若元素是"float 且落在
int48 值域"（`±0.0` / 次正规），会被 callout 的 int48 装箱启发式贴成 int ⇒ JIT `g=[0]` vs
NO_JIT `g=[0.0]` ✗（门禁探针 `jit_probes/probe_array_literal_float.leno`）。当时记的修法
"按 §8.116 给每个元素挂来源证明"—— 本次实施 ✓。

**为什么值域判据无解（这次彻底想清了）**：**任何 int48 raw 同时都是某个 double 的位型**
（小的 int48 = 次正规、大的 = 正规 double）⇒ "raw 落在哪一段"永远分不出"这个 raw 意思是 int
还是 double" ✗。§8.103 / §8.105 的守卫判据 `raw>>47 == 0`（§8.115/§8.116 已撤销）之所以既能
"命中一半"又能"漏一半"，根因正在这里 ✓。**类型必须来自产出指令的证明（或运行期类型位图），
不能来自值本身** ✓。

**修法：两条通道，都只增精度、不减精度**（`x86_64.c` emit 主循环 + `ops_callout.inc` + callout）

新增 `slot_float_prov[64]`（bit ⇒ 该 vstack 槽的 raw 必是 double 位型）与 `slot_from_si[64]`
（该槽由哪个局部量读取产出）；记账放在**每条指令收发码之后**（保守：净推 ≠ 1 或未声明 ⇒ 清零，
顶槽在净推 ≤ 0 时先清 ⇒ 不存在陈旧标记 ✗）：

- **静态通道**（编译期证明，零运行期开销）：`OP_CONST` 常量表里就是 float ✓；`OP_*_FLOAT` 六个
  **单路径**浮点 opcode（ADD/SUB/MUL/DIV/NEG/CAST）✓。**多路径 opcode（通用 `OP_ADD/SUB/MUL/DIV/MOD`）
  一律不置位** ✓（§8.116 的教训：只信单路径）。
- **动态通道**（运行期查 RBX 类型位图）：元素由 `OP_GET_LOCAL` 产出时用 `BT RBX, si` 取"该局部量
  非 int" ⇒ 对 int48 值域的 raw 即"是 float"✓（对象/null/bool 的 raw 是 NaN-boxed，**永不**落在
  int48 值域 ✓）。两个前提缺一即弃用（退回默认启发式，**不产生新错误**）：
  ① 该局部量**在循环体内不被写入**（否则位图是入口快照、会陈旧 ✗）—— 由新增的
  `local_written_mask` 前置线性扫描判定（长度未知的 opcode / `OP_CLEAR_LOCAL_RANGE` ⇒ 保守全 1）；
  ② `inline_depth == 0`（序言只为**调用方**的 `n = num_locals` 个槽建位图，callee 槽不在覆盖范围 ✗）。

**关键事实**：序言是**每个局部量逐槽按 tag 建位图**（`for i < n = num_locals`），且每次进入
JIT 区域都会重跑 ⇒ 对"循环内不被写入"的局部量，该位图在整个循环内都是当次入口的**精确**类型 ✓✓。

`jit_callout_array_new` 增第 3 参 `float_prov_mask`（bit i = **源码顺序**第 i 个元素）：置位的元素
**跳过** int48 装箱启发式（float Value 就是裸 double 位型 ⇒ `(Value)raw` ✓），未置位保持原启发式 ✓。

**实测（全绿）**

| 项 | 修前 | 修后 |
| --- | --- | --- |
| `probe_array_literal_float`（JIT）| `g=[0]`（NO_JIT `[0.0]`）✗ | **`g=[0.0]` = NO_JIT ✓**，且 **`Bailouts: 0`** ✓（循环仍在 JIT 里跑，不是靠"拉黑交解释器"）|
| 探针门禁全量（JIT vs NO_JIT，滤 JIT 统计块）| — | **IDENTICAL 41 / DIFF 9**；9 个 DIFF **全部是计时类探针**（数值断言部分逐字一致，例 `sum=320400 sum=800 seen=200` ✓）|
| `diag_array_literal_guard` / `diag_bailout_double_side_effect` | — | `Bailouts: 0` ✓ / `sum=201 len=201` ✓ |
| Bug5 复现件 / assert / crypto 五项（双模式）/ 飞机大战 600 帧 | — | `bad_count=0` ✓ / **312-0** ✓ / FAIL=0 ✓ / 跑完 + `Bailouts 0` ✓ |

**与 §8.105 守卫的对比（为什么这次能修对）**：守卫靠"值域 ⇒ 猜类型"⇒ 判据必错两半，且**命中就
bail**（解释器重做整轮、JIT 半轮副作用不回滚 ⇒ **数据污染**，见 §8.126）✗✗。本修法靠"产出指令 ⇒
证明类型"⇒ 判据精确 ✓，且**不 bail**（只在 callout 内改装箱方式）⇒ 零副作用风险 ✓✓。

**仍存的残余（如实记录）**：float 类型若**无法**由上述两条通道证明 —— `arr[i]`（容器取值）、
callout 返回值、**循环内被写入**的局部量、内联 callee 的局部量 —— 仍走默认启发式 ⇒ 这些形态下
`±0.0 / 次正规`元素仍可能被贴成 int ✗。要覆盖它们需要在**每个局部量写点**维护类型位图
（BTS/BTR，热路径成本）或给 callout 结果打类型标记 —— 未做（收益窄，风险与成本都明显更大）。

**教训**：

1. **类型只能来自"产出者"，不能来自"值"** —— 任何"看位型猜类型"的判据，在 float 与 int48 混存
   的设计里必然错一半（§8.103 / §8.105 / §8.126 / §8.127 这四处同族缺陷的总根因 ✓）。
2. **"零开销的精确"优先于"有开销的精确"**：能编译期证明的（常量、单路径 opcode）不要留到运行期；
   只有"循环内不被写入"的局部量才值得付运行期 `BT` 的代价 ✓。
3. **不 bail 的修法才是好修法**：同一个"类型分叉"问题，走 bailout 会变成数据污染（§8.126），走
   "只改装箱"则零风险 ✓ —— 以后遇到"JIT 与解释器语义分叉"，优先在 **callout 的转换点**修，
   而不是在**是否编译**上修 ✓。

***

### 8.128 【已修】数组字面量元素来自 callout 返回值（`[t[i]]`）—— 第三条来源通道；并**澄清写回路径无错值**（含 LICM 假绿教训）（2026-09-18）

#### 一、先纠一条我自己的误判：**写回路径没有错值**

上一轮据"位图只在序言建、写点不维护（`EMIT_STORE_LOCAL` 是裸存；全树无 `BTR`、`BTS` 只在序言）"
推测：局部量"入口非 int、循环内变 int"时，写回的 bit=1 分支会把**裸 int48 原样写进
`frame->locals`**（解释器按 double 读 ⇒ 静默错值）✗。**实测推翻**：

| 形态（`_jit_bugs/diag_writeback_stale_bitmap*.leno`）| 结果 |
| --- | --- |
| `z=1.0` → 循环内 `z=7` | 两模式 `7.0` ✓（编译器在赋值处插了隐式转换 ⇒ scratch 里本就是 double）|
| `b=true`→`b=7`、`d=5`→`d=1.5` | **编译期类型错误** ✗（语言"首赋定类型"，`float→int` 不合法）⇒ 该形态构不出 |
| `var a=null; a=D["b"]`(true) → 循环内 `a=D["i"]`(int 5) | `A=[5]` ✓（若 scratch 是裸 int48 则应打印次正规垃圾 ✗）|
| `var a=D["z"]`(0.0) → 循环内 `a=i`（**不可提升**）| `A=[299]` ✓（循环确实编译：`Compiled:1`）|

⇒ 结论：**静态 int/float 局部量的 scratch 表示与其静态类型一致**（类型系统挡住了危险转换：
`float→int` 编译期报错、`int→float` 隐式放宽时存进去的仍是 double ✓），**`any` 型局部量则按
Value（装箱）存** ⇒ 写回的两条分支（按 bit 原样存 / 按 tag 重新装箱）**都安全** ✓。位图确实只在
入口建、会"陈旧"，但陈旧**不产生错值** ⇒ 不需要为它做写点维护 ✓。

**⚠ 方法论教训（本轮最值钱的一条）**：上面前 5 个形态一度**全部**"与解释器一致"，我差点当成
"没问题"收工 —— 实际是**空结论** ✗：RHS（`7`、`D["i"]`、`D["z"]`）都是**循环不变量**，被
**LICM 提升出了循环** ✗✗。证据（`LENO_JIT_DEBUG=1`）：循环体只剩
`GET_LOCAL slot=1 / SET_LOCAL_POP slot=0 / FOLD slot=1`，那次 dict 取值根本不在循环里 ⇒
写回路径压根没被"循环内变型"碰到。**⇒ 写 JIT↔VM 分叉探针时，RHS 必须依赖循环变量**
（`a = i` 而不是 `a = 7`）否则测的是被提升后的常量 ✗。那几个文件保留作现场记录。

#### 二、已修：`[t[i]]`（元素是 callout 返回值）与解释器分叉

新门禁探针 `jit_probes/probe_array_literal_from_container.leno`（元素 = `t[i % 2]`，下标依赖
循环变量 ⇒ 不可提升）：

| 探针 | 修前 | 修后 |
| --- | --- | --- |
| JIT | **`g=[0]`** ✗ | **`g=[0.0]`** ✓（= NO_JIT ✓，`Compiled: 1 / Bailouts: 0` ⇒ 循环仍在 JIT 里跑）|

**修法（§8.127 的第三条通道）**：`EMIT_VALUE_TO_RAW()` 本来就在 tag 转换点上算"这个 Value 是不是
NaN-boxed int"（`shr 48 / cmp 0xFFFB`）⇒ 顺手加一条 `setne [rbp+ret_dbl_disp]`（**多 1 条指令**；
帧里 `frame_sz` 公式那多出来的 16 字节空闲槽放 1 字节 ⇒ 不改帧大小 ✓）；`OP_ARRAY` 对**紧邻的
上一条指令产出**的元素（`prev_ret_dbl`，与 §8.41 `prev_raw_int48` 同一套"一次性"语义）读该标志
决定装箱方式 ✓。

- **为什么精确**：标志是在**拿到 VM 的 Value（带 tag）**的那一刻算出来的 ✓ —— 值域判据永远
  分不出 `float 0.0`（位型全 0）与 `int 0` ✗（§8.105 / §8.126 / §8.127 的总根因）；
- **为什么安全**：置位 ⇒ 强制按 double 装箱，对 obj/bool/null（NaN-boxed 位型）是 no-op ✓；
  清零 ⇒ 完全走原启发式（与今天一致）✓；
- **覆盖面**：**只有最后一个元素**（单字节标志只描述最近一次转换；元素按源码顺序入栈 ⇒
  最后一个 = 紧邻 ✓）。与 §8.116 的"紧邻白名单"同一取舍：认得出就认，认不出退回原行为 ✓。

#### 三、"全量运行期类型位图"（根治方案）的性价比裁决：**不做**

- **成本**：每个局部量写点 1~2 条指令，且要覆盖**所有**写点 —— `SET_LOCAL` / `SET_LOCAL_POP` /
  `MOVE_LOCAL*` / `SET_LOCAL_CONST` / `INC·DEC_LOCAL*` / **§8.44 的寄存器折叠路径** /
  `CLEAR_LOCAL_RANGE`；而"存的这个 raw 是不是 double"未知时只能保守 `BTR` ⇒ 多数写点白付 ✗；
- **收益**：**正确性收益已被第一节的实测否证** ✗；只剩"放宽 §8.127 那条'循环内不被写入'限制"
  的**窄精度**收益；
- ⇒ 为窄精度收益付全热路径成本，**不值** ✓。真需要时优先做"**在转换点上顺手记类型**"这类零/低
  开销方案 ✓（本次 §8.127 + §8.128 即为范例：`BT RBX`、`setne` 都是搭既有测试的便车）。

#### 四、本轮验证（全绿）

| 项 | 结果 |
| --- | --- |
| `probe_array_literal_from_container`（新）| JIT `g=[0.0]` = NO_JIT ✓ |
...
#### 五、断言固件（本轮修复的防回归，2026-09-18）

本轮所有修复都落成 `assert/` 里的测试（套件自动发现 `test_*.leno`，无需改 runner ✓）。判据一律用
**绝对期望值**（⇒ 不依赖"JIT vs NO_JIT 对比"，**两种模式都必须 PASS**；这也顺带防住"测试绿了但
算法算错"）：

| 断言文件 | 钉住的修复 | 修前实测 | 现在 |
| --- | --- | --- | --- |
| `test_jit_gc_safepoint.leno` | §8.126 ②（GC：JIT 帧内禁止就地回收）| JIT 下 `bad_count=1`，**确定性第 57 轮（i=56）**（用户实测）| `BUG5_BAD == 0` 且 `BUG5_CAUGHT == 0` ✓ |
| `test_jit_bailout_side_effect.leno` | §8.126 ①（撤销 `OP_ARRAY` 歧义区守卫）| `G.len() = 204`（副作用多执行 3 次）✗ | `sum == 201` 且 `len == 201` ✓ |
| `test_jit_array_literal_types.leno` | §8.127（静态/局部量通道）+ §8.128（callout 通道）| 局部量形态 `g=[0]`、callout 形态 `g=[0]` ✗ | 三形态都 `"0.0"`，另有反面对照 `"0"`/`"7"` ✓ |
| `test_plane_war_headless.leno` | 飞机大战"启动 → 逐帧 → 退出"整链 | 驱动脚本 `_pw_auto.leno` 已删 ⇒ 门禁缺失 | `[PASS]`（dummy 驱动 + 120 帧）✓ |

套件总数：**312 → 313（+飞机大战门禁）→ 316（本轮再 +3）**，全绿 ✓。

**写这类断言的三条要点（本轮踩过 / 验证过的）**：

1. **RHS 必须依赖循环变量** —— 否则 LICM 会把循环不变量提升出循环，测试变**假绿** ✗
   （§8.128 第一节的教训：5 个形态全空结论）；
2. **判据用"绝对期望值 + 字符串形态"**：`"" + x` 对 int / float 分别给出 `"0"` / `"0.0"` ⇒
   这是抓"类型分叉"最便宜的判据 ✓（比引入类型内建更省事）；
3. **不要用"只有 JIT 才有的行为"去断言 JIT 生效** —— 套件读不到 JIT 统计（走 stderr）；
   靠"循环足够热（数百轮）+ 绝对期望值"即可 ✓（JIT 默认开启 ✓）。

***

### 8.129 【已修复】所有 GUI 应用在 JIT 下闪烁 / 控件消失 —— 真因：JIT 的 **cstruct 字段读**每轮 bailout ⇒ 整轮重放 ⇒ 副作用执行两遍（2026-09-18）

**来源**：用户实测（本机真实窗口）——「**所有** GUI 应用示例在 JIT 下闪烁、控件一个个消失、点击不响应」，
而 `LENO_NO_JIT=1`（VM）**完全正常** ⇒ 一开始就排除了渲染/库层 ✗。

#### 定位：二分（每个锚点只改 `src/` + 重编译，`lib/` 全程不动 ⇒ 用户手动跑真实窗口判定）

| 锚点 | 时间 | 结果 |
| --- | --- | --- |
| 09-18 HEAD | 今天 | **闪** ✗ |
| `bd1a8979` | 09-17 23:50 | **闪** ✗ |
| `3c9618c7` | 09-17 20:26 | **闪** ✗ |
| `e347e424` | 09-17 07:26 | **闪** ✗ |
| `b0f0f6b6`（54 提交的中点）| 09-16 07:37 | 不闪 ✓ |
| `42e2810c` | 09-16 21:15 | 不闪 ✓ |
| `6c73aa8f` | 09-16 23:49 | 不闪 ✓ |
| `858367a2`（49~54 中点）| 09-17 01:05 | **闪** ✗ |
| `62495117`（49~52 中点）| 09-17 00:23 | 不闪 ✓ |
| `124a3e4e`（51/52 二分）| 09-17 00:31 | 不闪 ✓ |

⇒ 元凶唯一锁定 = **`858367a2`（R6-k：native 绑定方法 `T.malloc()` / `c.free()` 进 JIT）** ✓。
它提交信息里那句 **"fm 同条件 A/B 1.79x（改前 ≈ NOJIT，热循环原被拉黑退化成解释执行）"** 正是机制说明 ✓。

#### 机制（与症状逐条对上）

控件热循环里全是 **native 绑定方法**调用（实测诊断行：`obj_type=10` = `OBJ_BOUND_METHOD`、
`has_recv=1`、`total=2` ⇒ 形如 **`Array.add(x)`** ✓）。R6-k 之前这些调用一律 `failed → bailout`
⇒ **交解释器执行**（慢但正确 ⇒ VM 模式永远干净 ✓）；R6-k 之后它们**留在 JIT 里原地直调** ✓，
而这条新路径只要有一处与解释器语义不符，就会写坏控件状态 ⇒ **消失 / 闪烁 / 不响应** ✗。

#### 本轮处置：先"回退"（把性能代价如实记下，**真修复见下方待办**）

`src/jit/jit_callout.c` 里 R6-k 新增的 `if (nf) { …原地直调… }` ⇒ 改回 `jit_callout_failed = 1; return NULL_VAL;`
（= R6-k 之前的行为 ✓，识别与诊断保留：`LENO_JIT_DEBUG=1` 时打印是哪类 callee 触发的回退 ✓）。

| 代价 | 数据 |
| --- | --- |
| fm 帧耗时 | 回退后 **15.2 ~ 16.8 ms/帧**（确定性驱动 `LENO_SDL_FRAMES=300`，3 轮，`sum=23700` 逐轮一致 ✓）；R6-k 开启时按它自己的同条件 A/B 为 **1.79x**（≈ 8.5~9.5 ms/帧）|
| 飞机大战 | 重新出现 bailout ⇒ `assert/test_plane_war_headless.leno` 的 `Bailouts: 0` 判据失败 ⇒ 本轮临时改为**可选严格模式**（`set LENO_GATE_STRICT=1` 才判 0 ✓）——**这是临时状态，修好必须恢复** ✓ |
| 其它应用 | 不受影响 ✓（crypto 五项 FAIL=0 ✓；assert 316 条中仅上述 1 条 ✓）|

#### 已排除的假设（都有数据，避免以后重走）

| 假设 | 判据 | 结果 |
| --- | --- | --- |
| 当天改的 `OP_ARRAY` 装箱污染数组数值（§8.127/§8.128）| `_jit_bugs/diag_array_value_corruption.leno`（float / int / 变量 / 下标四形态）| JIT 与 NO_JIT 校验和**逐字相同** ✓ 排除 |
| 当天改的 GC（§8.126 JIT 帧内不回收）⇒ 内存暴涨 / 卡顿 | 同一 app 无头 22 s | JIT **+0.6 MB** / NO_JIT +0.0 MB ✓ 排除 |
| 确定性渲染被 JIT 画坏 | 像素回读存 BMP 比哈希（60 帧，`_jit_bugs/diag_gui_pixel_probe.leno`）| 两模式**逐字节相同**（`62B904B8C7CB7939`，360054 B）✓ 排除 |
| 比较 / 分支（命中测试、hover、`OP_CMPJMP_*`）| `_jit_bugs/diag_cmp_branch_stress.leno` | 两模式**逐字相同** `hits=1186368 acc=102975000.0` ✓ 排除 |
| `lib/` 被改（渲染策略等）| mtime 全为 09-17 11:45（本轮一行未动 ✓）| 排除 ✗ |

#### 待办：**真修复（不绕过）**

1. **首要线索**：§8.82 / §8.87 早就怀疑 —— "**`OP_INDEX` 慢路径 pop 了 2 个槽之后 `TOS_PRODUCE()`
   只把结果 pin 在寄存器、没补回物理栈 ⇒ 之后的 `rsp` 相对寻址偏移**" ✗。R6-k 的直调正是**从
   `vstack_top` 取实参** ⇒ 布局一错 ⇒ `arr.add()` 收到**错值** ⇒ 控件状态被写坏 ✓✓（与症状完全吻合）。
   **无头可验证**：`a.add(b[i])`（实参来自容器取值 ✓）形态 + JIT/NO_JIT 逐字对照 ✓。
2. 其次核对：接收者插 `args[0]` 的顺序 ✓、异常路径 ✓、`vm->sp` / `frame_cnt` 恢复 ✓，
   以及 `stop_frame_cnt` / `current_exec_vm` 的嵌套语义（`vm_call_value` 对此有专门保存/恢复 + 注释 ✓，
   直调路径没有 ✗）、native 重入时 JIT 侧未入根的 callee / receiver ✓。
3. 修好后：重新打开本路径 ✓、**恢复** `test_plane_war_headless.leno` 的 `Bailouts: 0` 判据 ✓，
   并新增一条"控件库形态（`Array.add` 热循环）"的 JIT/NO_JIT 对照门禁 ✓（本轮的诊断行可直接当探针锚 ✓）。

#### 教训

1. **"让更多代码进 JIT"本身就是一类风险**：R6-k 的 1.79x 收益是**真实的** ✓，但它同时把一批
   从未在机器码里跑过的调用点**首次**推进 JIT ⇒ 任何语义细节不符都会变成**静默污染** ✗。
   凡"解锁某类调用进 JIT"的改动，**必须**配一条"该形态在真实应用里逐字一致"的门禁 ✓。
2. **二分要贯彻"一次只动一个变量"**：本轮每个锚点**只拉 `src/` + 重编译**（`lib/` 保持不变 ✓），
   于是"库 vs 引擎"这条岔路从一开始就被排掉 ✓，10 个锚点即落到单个提交 ✓。
3. **用户可观察症状是最好的判据**：闪烁 / 消失 / 不响应这种"看起来像渲染问题"的表象，
   根因在解释器 / JIT ✓ —— 先用 `LENO_NO_JIT=1` 一刀切开"引擎 vs 库"，比任何推测都省时间 ✓。

#### ★ 真因与真修复（同日完成，**不绕过**）

**真因（设计缺口，不是 R6-k 写错）**：`jit_callout_get_field` **只实现了 `OBJ_STRUCT` 分支**
（源码注释自陈："cstruct … 一律 `failed → bailout`，交解释器重放本条指令"✗），而 GUI 热循环里同时有
① **cstruct 字段读**（`pt.x` / `ev.type` ⇒ 这一段**每轮都 bailout** ✗）与 ② **native 绑定方法调用**
（R6-k 之后**不再 bail** ✓）⇒ ②把循环**留在 JIT**、①让它**每轮 bailout + 解释器重放整轮** ⇒
同一轮里的**堆侧副作用被执行两遍** ✗ ⇒ 控件列表被反复改写 ⇒ 用户看到的"控件一个个消失 / 闪烁 /
点击不响应" ✓✓。（R6-k 之前两者都 bail ⇒ 3 次后**拉黑**整段交解释器 ⇒ 最多错 3 次 ⇒ 看不见 ✗——
这就是二分能指到 R6-k、但它不是真凶的原因 ✓。）

**取证**（`LENO_JIT_DEBUG=1` + **真实窗口** + 用户拖鼠标；日志 4.2 MB / 13982 行）：

```
[FIELD-FAIL] get_field: obj 不是 struct（bits=0xfffc0276cd5959a0, type=17）    × 291
[JIT-CALLOUT] call_value 委派 VM 重入: is_obj=1 obj_type=10 arg_count=1       × 291
```

`type=17 = OBJ_CSTRUCT` ✓；两组计数**严格相等**（一一对应 ✓）；哨兵 `★异常` = **0** ⇒ 既不是
"接收者错值" ✗、也不是异常路径 ✗ —— 就是这条**设计缺口** ✓。

**修复**：给 `jit_callout_get_field` 补 **cstruct 分支**，语义照 `vm/vminc/op_struct.inc:665-728`
（唯一来源）逐条搬：越界 / 找不到嵌套定义 / 分配失败 ⇒ 仍 `failed → bailout`（报错原文交解释器 ✓）；
`str16 + array_dim>0` ⇒ `cstruct_get_field_value`（UTF-16→UTF-8 ✓）；`array_dim>0` ⇒ 新建
`ObjCStructArrayView` ✓；`TYPE_CSTRUCT` ⇒ 新建指向父结构体内存的嵌套实例（`owns_memory=0`）✓；
其余 ⇒ `cstruct_get_field_value` ✓。

**验证（R6-k 保持开启 —— 即性能没有回退）**：

| 项 | 绕道方案（回退 R6-k）| **本修复** |
| --- | --- | --- |
| 用户实测（真实窗口 + 鼠标）| 不闪 ✓ 但性能退 ✗ | **不闪 ✓ 且性能保留 ✓** |
| fm `FIELD-FAIL` / `CALLOUT-FAIL` | — | **0 / 0** ✓ |
| fm `Bailouts` | 非 0（热循环被拉黑）✗ | **0** ✓ |
| fm `us_per_frame` | 15.2 ~ 16.8 ms ✗ | 见 §8.130 实测（回到 R6-k 水平 ✓）|
| `assert` 全套 | 316-0，但飞机大战那条靠**临时放宽** ✗ | **316-0，且门禁恢复强制 `Bailouts: 0`** ✓✓ |

⇒ `assert/test_plane_war_headless.leno` 里的 `LENO_GATE_STRICT` 临时开关**已删除**、恢复"必须 0" ✓。

#### 教训（补充）

4. **"解锁 JIT 覆盖面"必须配套"清掉同循环里的残余 bailout"**：R6-k 本身没有错 ✓ —— 它只是让循环
   留在 JIT，从而把另一条 bailout 路径的"整轮重放"从"3 次后拉黑"放大成"每轮都重放" ✗。
   只要 JIT 里存在**稳定 bailout** 的指令，同轮有堆侧副作用就有"执行两遍"风险 ✗ ——
   这正是"零 bailout 门禁"的价值 ✓✓（飞机大战那条门禁早就在盯这件事 ✓）。
5. **哨兵式仪器 > 全量日志**：把"绑定方法接收者必须是对象"做成 `★异常` 哨兵 ⇒ 0 命中即排除一整类
   假设 ✓；打印策略用"抽样 + 异常形状立即打印"把日志压到 4 MB（全量会几十 MB，还会拖慢应用、
   反过来干扰被测现象本身 ✗）✓。
6. **二分指到的"变更"未必是"缺陷"**：`858367a2` 确实是"症状出现的那次变更"✓，但它本身是对的 ✓ ——
   真凶是它**暴露**出来的旧缺口 ✓。⇒ 二分之后**必须再问一句"它改变了什么执行路径"** ✓，
   而不是直接回退它 ✓。

***

### 8.130 §8.129 修复后的性能与健康度实测（R6-k 保持开启）（2026-09-18）

同机、同一确定性驱动（`LENO_SDL_FRAMES=300` + `SDL_VIDEODRIVER=dummy`；`sum=23700` 逐字一致
⇒ 两次的**渲染工作量相同** ✓，比 `us_per_frame` 才有意义 ✓）：

| 构建 | fm `us_per_frame` | fm `Bailouts` | 飞机大战门禁 |
| --- | --- | --- | --- |
| 临时**回退** R6-k（绕道方案）| 15.2 ~ 16.8 ms（3 轮）| 非 0（热循环被拉黑 ✗）| 需临时放宽 ✗ |
| **§8.129 修复**（R6-k 开启 + cstruct 字段读进 JIT）| **13.86 ms** | **0** ✓ | `Bailouts: 0` **强制通过** ✓ |

其余回归（全部通过 ✓）：`assert` **316 passed / 0 failed** ✓；crypto 五项 `FAIL=0` ✓；
`_jit_bugs/bug5_aes_int_array_float` `bad_count=0` ✓；新探针
`jit_probes/probe_native_arg_from_index.leno` JIT == NO_JIT（`acc=373750 len=2000 len2=500` ✓）。

⚠ 口径说明：fm 帧耗时里含渲染（FFI/GPU）⇒ JIT 收益有上限 ✓；本表只作"修复前后同口径"的对比，
**不**与 §8.88 的 1.79x 直接换算（那次的对照基线是被拉黑后的解释执行 ≈ `NO_JIT` ✓）。

***

### 8.131 §8.48「类型化形参快路径」的**收口**：找到上一版"加了 tag 确认却无效"的真因，并**整块删除**该路径（2026-09-19）

**起因**：§8.125 对该快路径的处置是"**停用**"✗ —— 把"静态 `int` ⇒ 值是 int48"这个错误
假设**留在代码里、只是不执行** ✗（用户对这类"半修"状态的评价是"不三不四"，属实 ✓）；
且当时把代价记为 `nlev_f_intstep_loop` 约 **+10%** ⇒ 本轮做"提速回收" ✓。

#### 一、上一版"加了 tag 确认却实测未生效"的真因 ✗ —— **补丁顺序**

正确的字节序必须是：

```
[jne 通用路径][快路径体][jmp .next][通用路径][.next]
```

上一版写成"先把 jne 补到当前 `cb->len`、**再** emit 快路径的跳过 jmp" ⇒ **jne 正好落在
那条 jmp 上** ✗ ⇒ 非 int 实参（bigint / float / 对象 / null / bool）**跳过通用路径** ⇒
scratch 槽留着**残留值** ⇒ 静默错值 ✗。
（这与更早 `continue` 版"通用路径的机器码根本不生成" ✗ 是**两个不同的错** ✗ ——
上一轮只修了后者，所以现象"没变"✓。）

本轮实现（已验证正确 ✓）：**先** `emit_jmp`，**再** `patch_rel32(jne → cb->len)` ✓。

#### 二、为什么最终是"删除"而不是"保留" ✗ —— 实测收益 ≈ 0

`_jit_bugs/bench_typedparam.leno`（两个极小函数体 ⇒ 序言占比最大；各 5 次取中位）：

| 形态 | 快路径 **开** | 快路径 **关** | 结论 |
| --- | --- | --- | --- |
| 2 形参（`tiny`，1e7 次调用）| **18121** | **17327** | 开 = **慢 4.6%** ✗ |
| 6 形参（`wide`，序言 ×3）| 34265 | 34575 | +0.9% ✗ |

两者与 ±5% 噪声同量级、**方向还相反** ⇒ **效应 < 噪声** ✓。
**逻辑上也必然如此**：既要确认 tag（4 条指令）就必须把通用路径一并发出 ✗ ⇒ 净省的只有
通用路径里那一条 `and` ✗（单指令级）；而本项目真正的热负载（sha512 / TraeSign 的
`add64` / `pack64`）形参是 **bigint** ⇒ 检查**必失败** ⇒ 这条路径是**纯加开销** ✗。

⇒ 按 §8.128 的先例（"为窄收益付全热路径成本 ⇒ 不值 ⇒ 不做"✓）**整块删除** ✓：
序言只剩**唯一一条通用路径**（按 tag 分流：int48 取载荷 ✓ / float·对象·null·bool 原样存 +
置 RBX 位图 ✓）—— 让"静态 `int` ⇒ int48"这个**假设从代码里消失** ✓，比"给它加一层
检查"更彻底 ✓。`LENO_NO_TYPEDPARAM` 一并删除 ✓。

**将来若要拿回这点指令**：方向是**调用点特化**（把"实参全是 int48"作为**调用点**信息带进
被调方 ✓），而不是在序言里对每个形参重付一次检查 ✗。

#### 三、回填一条**归因错误**（诚实记录 ✗）

§8.125 记的"停用该快路径 ⇒ `nlev_f_intstep_loop` 93~97ms → 104~122ms（约 +10%）"
**不成立** ✗ —— 那个探针是**循环 JIT**（`ctx->func_mode == 0`）⇒ **根本进不了 §8.48**
（快路径有 `ctx->func_mode` 守卫 ✓）⇒ 那 10% 属**噪声 / 机器负载** ✗，不是这条路径的收益 ✓。
**教训：归因之前先确认"被研究的代码在这条路径上真的会被执行到"** ✓
（本轮是靠 `bench_typedparam` 直接打靶才发现的 ✓）。

#### 四、本轮验证（全绿 ✓）

| 项 | 结果 |
| --- | --- |
| `assert/test_jit_bigint_arith.leno`（§8.125 门禁：200 次热调用 + 3 元 Σ）| JIT `OK` ✓ / `LENO_NO_JIT=1` `OK` ✓ |
| `_jit_bugs/diag_add64_many.leno`（第 50 次调用起）| i=45..55 全 `true` ✓ |
| `examples/crypto/sha512.leno`、`sha256.leno` | JIT 3/3 + NO_JIT 3/3 全 PASS ✓ |
| 五枚探针（`probe_bigint_const_ops` / `probe_bitwise_bigint_sign` / `probe_arr_add_loop` / `nlev_e_floatstep_loop` / `nlev_f_intstep_loop`）| JIT 与 NO_JIT **逐字 IDENTICAL** ✓ |
| 完整门禁 `assert/run_tests.leno` | **334 passed / 0 failed** ✓ |

删掉的那段代码在"§8.125 停用版"里因条件恒假而**从不发射** ✓ ⇒ 本轮**运行时行为不变** ✓，
变的只是"树里不再留死代码 + 错误假设" ✗✓。

***

### 8.135 让 JIT 真正覆盖大整数：snapshot 式侧退出 + 旁路（**设计稿，尚未动手**，2026-09-19）

**为什么是这条路**：§8.134 的账本 + 逐段实测确认了两件事 ——
① crypto 的差距出在**分配**（小 bigint 每操作 3 次堆分配 / 144 字节 ✗，int48 是 0 ✓），不在"任意精度运算"；
② 在 bigint 循环上 **JIT≈解释器（B 段 438607 vs 444821 µs，1.4%）** ✗ —— 因为 JIT 遇大整数就 bailout，
bailout 满 3 次**整条循环被拉黑** ⇒ 之后一直跑解释器 ✓（`Bailouts: 3 / Executed: 3` ✓ 就是这个意思）。

**§8.134 的失败留给本设计的三条硬约束**（照 V8 deopt / LuaJIT snapshot+side exit / PyPy guard+resume 的做法 ✓）：
1. **虚拟栈没有类型通道** ✗：`EMIT_RAW_TO_VALUE`（`x86_64.c:678-697`）只能靠"位型启发式 + RBX 位图"猜 ✓ ——
   而"静态 int"既可能是裸 int48 也可能是 `0xFFFC|ptr` 的 bigint（§8.125 ✓）⇒ 装箱值与裸值在栈上**不可区分** ✓。
2. **JIT 的槽位值域是静态假定** ✓：`x86_64.c:289`（折叠）与位图说明 `x86_64.c:665-676` 都写着"操作数假定为 int48，
   由扫描器保证" ⇒ 一旦某个值**运行期**变成装箱 bigint，写回/后续消费会按裸 int48 处理 ⇒ **静默错值** ✗
   （实测：`assert/test_bigint.leno` 的 `fib_bigint(100)` 期望 354224848179261915075，实际每次不同的垃圾 ✓）。
3. **bailout 是"整轮重放"** ✗：`x86_64.c` 的 bailout 块（站点桩见 `:1642-1650`）写回 locals 后由
   `jit_try_hot_loop` 回到 `OP_LOOP`（`vm/vminc/op_jump.inc:39-76`）从**循环头**重来 ✓ —— 迭代中途退出即
   **重放整轮**（§8.31 记过的隐患 ✓）。真正的侧退出必须在**失败的那条 op** 续跑 ✓。

**分步（每步独立可验 ✓，不留半成品 ✗）**：

| 步 | 做什么 | 靶子（必须全绿才进下一步 ✓） |
| --- | --- | --- |
| **S1** | **虚拟栈类型通道**：给 JIT 的 vstack 每槽一位（或按站点 ✓）标记"裸 int48 / 装箱" ✓；`EMIT_RAW_TO_VALUE` 与各消费者改读它 ✓；**语义零变化** ✓（先只做"知道" ✓，不做"利用" ✗） | `assert/run_tests.leno` 335/0 ✓、sha512/sha256 ✓、`test_bigint` ✓、五探针 JIT==NO_JIT ✓ |
| **S2** | **侧退出码 5 + 精确续跑**：新增返回码 ⇒ 解释器用已有的 `jit_bailout_site`（`x86_64.c:528-541` ✓ 记录的就是失败处的 bc 偏移 ✓）把 `frame->ip` 定位到**那条 op** ✓（不再从循环头重放 ✓）；退出时按现有写回路径把 locals + vstack 物化一致 ✓ | 新写一个"迭代中途退出**不重复副作用**"的门禁（计数型 ✓）+ 全套 ✓ |
| **S3** | **旁路（bridge）= 侧退出后重编译**：记录"该站点见过装箱值" ✓ ⇒ 第二次编译给该站点发**装箱路径**（= §8.134 那个 callout 宏 ✓，但此时下游有 S1 的类型通道 ✓ 不会再被当裸值 ✗） | `_jit_bugs/bench_attrib_bigint.leno` 的 B/D/E：`Bailouts` 降为 0 ✓ 且**变快** ✓；`assert/test_bigint.leno` ✓；crypto 3/3 ✓ |
| **S4** | 去掉"3 次即拉黑"✓（改为侧退出重试 N 次后**保留**已编译代码 ✓），并量化 | 账本四类负载重测 ✓（`_jit_bugs/bench_ledger.leno` ✓） |

**明确不做**（§8.134 的教训 ✓）：**在 S1 之前**再往快路径塞装箱结果 ✗；没有精确续跑点就开侧退出 ✗；
靠"加检查"代替类型通道 ✗（那会给出题所述"全体加税"✗ 且收益≈0 ✓）。

**参考实现要点（可偷 ✓）**：V8 的 feedback vector + deopt 元数据（"此处状态 = 哪个值是哪个" ✓）；
LuaJIT 的 snapshot + side exit（旁路**回到主循环** ✓ 不是作废 ✓）；PyPy 的 guard + 惰性 bridge（换类型的
那个分支**也有自己的优化代码** ✓ = S3 ✓）。
...
### JIT 账本（2026-09-19）：四类负载上的 JIT vs 解释器

**问题**（用户直接提的）："JIT 优化了这么多，**性能提升看不到**" ✓ —— 这是可以量出来的 ✓。

**方法**（都遵守既有度量纪律 §8.97 / `jit_probes/README.md` 的"确定性驱动 + 交替 + 中位数"）：

- 同一个二进制，只切 `LENO_NO_JIT=1` ✓；**JIT / 解释器交替跑**各 3 次、取中位 ✓
  （避免"越跑越慢"的系统偏差 ✓）；
- 每条负载都有**正确性锚点**（两种模式下必须逐字一致 ✓）：`832040` ✓ / `crypto_sum=388000` ✓ /
  `parse_bad=0 parse_acc=22600` ✓ / `sum=21600` ✓ —— **全部逐字相同** ✓；
- GUI 用**无头确定性驱动**（`SDL_VIDEODRIVER=dummy` + `LENO_SDL_FRAMES=300` ✓，§8.73 的老办法 ✓），
  读 `us_per_frame` ✓。

| 负载（工具）| JIT（3 次）| 解释器（3 次）| JIT/解释器 | 判读 |
| --- | --- | --- | --- | --- |
| **数值**：`fib(30)` 递归（`examples/bigint/fib_recursive_bench.leno`）| 31.97 / 31.43 / 29.29 ms | 79.58 / 74.97 / 73.65 ms | **2.44×** ✓ | JIT 真正赚钱的地方 ✓ |
| **crypto**：SHA-512(64B) × 2000（`_jit_bugs/bench_ledger.leno`）| 16.46M / 16.57M / 16.34M | 16.07M / 16.02M / 16.09M | **0.98×** ✗ | **无收益**（JIT 略慢 ✗）|
| **解析**：XOR 串解码 4000 个（同上）| 6955 / 8018 / 7072 | 8951 / 8348 / 8297 | **1.18×** ✓ | 小赚（callout 密集的字节循环 ✓）|
| **GUI**：`file_manager` 300 帧（无头）| 6888 / 6913 / 6940 µs/帧 | 6753 / 6884 / 6677 µs/帧 | **0.98×** ✗ | **三轮全部略慢** ✗ |

**为什么 crypto / GUI 是 0.98×**（有据 ✓）：crypto 那次 JIT 统计为
`FuncCompiled=19, FuncExecuted=19 518 402` ✓ —— 机器码**确实在跑** ✓，但 SHA-512 的实现以
**bigint** 值为主（`0x8000000000000000` 及以上字面量 ✓）⇒ JIT 的 int48 快路径**整体退避**、
真实工作走 callout ✓ ⇒ 序言/装箱的额外成本盖过了收益 ✗。GUI 同理：热路径是 callout（SDL 调用）
+ 对象/字符串 ✓，不是数值循环 ✓。

**结论（JIT 的定位）**

1. JIT 是**数值热循环 / 递归加速器**（2.44× ✓），**不是应用加速器**（crypto、GUI 均 ≈ **0.98×** ✗）；
2. ⇒ 后续**不应**为了"让应用更快"继续加 JIT 特性 ✗ —— 本轮 §8.131（形参快路径，实测 ≈0 ✗）、
   §8.95（加大内联上限，实测 ≈0 ✗）、§8.128（位图，窄收益 ⇒ 不做 ✗）都是同一结论 ✓；
3. 而且**恰是这些"≈0 收益"的特性贡献了本轮全部静默错值** ✗（§8.132 内联多返回点、
   §8.125① 形参快路径 ✓）⇒ **收益 ≈0、风险最高的那部分，最该冻结** ✓
   （即把"宁可不编，不要猜"从纪律变成默认 ✓）；
4. 应用侧真正值得动的是 **callout / 分配 / 字符串 / 大整数**（crypto 与 GUI 的公共瓶颈 ✓）——
   与 JIT 无关 ✓。

**本次测量的注意事项**（诚实标注 ✓）

- 机器为本机 i5-3450（Ivy Bridge）✓，绝对值**不可跨机**比较 ✓；
- `times.us()` 在这个构建里给出的量级偏大（crypto 项 ~1.6e7 ✓）⇒ **只作同机 A/B 的相对量** ✓，
  不要当微秒读 ✓；
- GUI 口径按既有记录的不确定度为 **±1.5%**（`sum` 四次逐字相同 ✓）；本题**三轮同向** ⇒
  方向可信 ✓、幅度按 ~2% 读 ✓。
...
## 15. JIT 不变量总表（整理：2026-09-20）

> 为什么要这一节：本 JIT 是「**单遍发射器 + 无类型虚拟栈**」✗，正确性靠一堆**隐式不变量**撑着 ✓。
> 这些不变量过去只以长段注释散落在 `x86_64.c` / `ops_*.inc` / `jit_callout.c` 里 ✗ ——
> 改一行就可能违反某条自己没读过的规则 ✗。本节把它们**集中成表** ✓；每条都给 `文件:行号`。
> 源码里的 §-编号注释只保留"规则 + 为什么"✗，历史过程（哪一轮改成什么）留在提交信息与本文件第 8 节 ✓。

### 15.1 类型通道（谁写 `vt[]`、何时作废）

| 项 | 规则 | 证据 |
| --- | --- | --- |
| `vt[]` | `uint8_t vt[256]`，`JIT_VT_MAX=256`，起步全 `VT_UNKNOWN` | `backend/x86_64.c:633,638` |
| `VT_*` | `UNKNOWN=0 / INT48=1 / DOUBLE=2 / VALUE=3`（`VALUE` = 已是 NaN-boxed Value） | `backend/x86_64.c:634` |
| `tos_type` / `tos_live` | `tos_live=1` ⇒ TOS 在 RAX 且类型在 `tos_type`；否则在内存栈顶且类型在 `vt[vstack-1]` | `backend/x86_64.c:619,674,699` |
| `tos_pending` | **一次性**待生效标注：产出指令在 `TOS_PRODUCE()` **之前**设，PRODUCE 取走后**立刻清回 UNKNOWN** ⇒ 未标注的产出自动是 UNKNOWN，绝不继承陈旧值 | `backend/x86_64.c:675,718,1912` |
| **允许写 `vt[]` 的 9 处** | ①整块初始化 ②`TOS_SPILL` 写 `vt[vstack-1]` ③`TOS_PRODUCE` 写 `vt[vstack]` ④`VT_INVALIDATE_MULTI_RET` ⑤`VT_SET_MULTI_RET_TYPES` ⑥`TOS_PRODUCE_FROM_RDX` ⑦`TOS_CONSUME_RAX/TO` **清**该槽 ⑧内联返回整块清 ⑨合并点求交/整块清 | `x86_64.c:638,710,728,743,765,789,809,1883,2014` |
| ⚠ 写入禁忌 | **不得**用 `tos_type` 之外的东西重写 `vt[槽]` ✗ —— `TOS_SPILL()` 会用 `tos_type` 覆盖它（上一轮折叠失败的根因） | `x86_64.c:2078` |
| 整块作废的触发点 | 前向跳转目标 / 合并点逐入边求交（只覆盖前 64 槽，更高槽判不一致）/ **回边目标**（直接整块清）/ 回边表溢出 / 内联返回 / 每槽消费即失效 | `x86_64.c:1999,2000,1648,1880,807` |
| `local_vt[]` | **按 scratch 下标**跟踪局部量编译期类型；白名单写点（`SET_LOCAL*/MOVE_LOCAL*`）由扫描算出可信掩码 `local_untracked_mask`；不可信 / 内联中 / `si≥64` ⇒ UNKNOWN | `x86_64.c:643,665,668,1651` |
| RBX 位图 | **运行期**"该局部量非 int48"位图（序言建立），与上面的**编译期** `local_vt[]` 是两套东西 ✓ 别混 | `x86_64.c:1266,1321,1580` |
| `slot_from_si` | 第二通道：记录"这个栈槽来自哪个 scratch 读" ⇒ 供 RBX 位图动态查询 | `x86_64.c:1586,2080` |
| `OP_SET_PTR_ELEM_TYPE` / `OP_SET_DECLARED_FACE` | 都不弹不推、无失败通道、**不写 `vt[]`/`local_vt`**；只给 TOS 上的 `ObjFFIPointer`/`ObjStruct` 补类型，装箱依据取通道 TOS 类型 | `ops_callout.inc:634,650,640` |
| ⚠ 通道的失效边界 | 类型通道在**跳转目标/合并点**会整块失效（保守 ✓）⇒ 任何"跨合并点仍成立"的证明**不能**只靠 `vt[]` ✗（历史上 `OP_ARRAY` 元素来源为此踩过红 ✓） | `ops_callout.inc:1282` |

### 15.2 bailout / 拒收（三条通道，别混）

| 通道 | 位置 | 行为 |
| --- | --- | --- |
| **静态拒收** | `jit.c:50,390`（`scan_loop_body` / `func_body_is_simple`）| 记 `jit_gaps` 直方图后 `return 0`，机器码不产出 ✓ |
| **运行期 bailout** | 机器码返 1 ⇒ `jit.c:870` | `bailout_count++`；`>=3`（`JIT_BAILOUT_LIMIT`）⇒ **永久拉黑** |
| **禁用重编一次** | ①普通 bailout（`site>=0`）且 `avoid_fold_cnt<4` ⇒ 记站点、禁用该站点折叠、`tried=0` 重编 ②入口守卫失败（`site<=-2000`）⇒ `guard_disabled=1`、`tried=0`，**不拉黑** | `jit.c:871,889` |

**`site` 编码约定**（四个地方同述，改一处必须改四处 ✗）：`>=0` 溢出/截断类（值 = 触发指令的 `bc_off`）；`-999..-1` 序言；`-1000..-1999` 其它（`bc_off = -1000-site`）；`<=-2000` 入口守卫失败（槽 = `-2000-site`）。
证据：`jit.h:79`、`x86_64.c:1084`、`jit_callout.c:59`、`jit.c:960`。

**`EMIT_BAILOUT_SITE_*` 五条顺序约束**（违反 ⇒ 站点记错/漏记/性能假象 ✗）：

1. **站点写入必须放在反向块入口，不能放在 `jcc` 之前** ✗ —— 宏会写全局站点并可能用 RAX ⇒ 冲掉 `test` 用的步进值（实测那样改 AES 仍红）✓ `ops_loop.inc:59,85`
2. `EMIT_BAILOUT_SITE_WRITE` 必须被**紧随其后**的 `patch_add(...,-1,...)` 消费成桩记录；未消费的 pending 会被丢弃，分支直连 bailout 块时打 `[BAIL-NOSITE]` ✓ `jit_priv.h:267`
3. 桩表（≤512）满 ⇒ **拒绝编译**，绝不静默丢站点 ✓ `jit_priv.h:272`、`x86_64.c:2316`
4. 站点桩收尾**统一发射**（此处可随便用 R9 —— 马上 bailout）✓ `x86_64.c:2247`
5. 非溢出编码与守卫区间**可能撞号** ✗ ⇒ 见 15.5 缺口 #2 `x86_64.c:1095`

### 15.3 寄存器与栈记账

| 寄存器 | 角色 | 证据 |
| --- | --- | --- |
| RCX | `locals`（第一参，全程保留） | `x86_64.c:1276` |
| R9 | `globals` 基址（RDX→R9） | `x86_64.c:1319` |
| R10 / R11 | `JIT_PAYLOAD_MASK` / `JIT_INT_TAG`（callout 后由 `EMIT_CALLOUT_END` 重载） | `x86_64.c:1315,1209` |
| RBX | 类型位图（bit=1 ⇒ 该局部量非 int48），callee-saved | `x86_64.c:1266,1321` |
| RAX | TOS / 结果 | `x86_64.c:701` |
| RSP / RBP | 虚拟栈指针 / 帧指针 | `x86_64.c:1277` |
| R12–R15 | pin 寄存器（callee-saved，按序 push/pop） | `x86_64.c:1263` |
| RSI / RDI | 序言保存（Win64 非易失；JIT 借来当 callout 参数寄存器） | `x86_64.c:1267` |
| XMM0/1/3 | 浮点左/右操作数、常量 0.0 | `x86_64.c:1115` |

**会被破坏的宏**：`EMIT_RAW_TO_VALUE_T` / `EMIT_VALUE_TO_RAW` / `EMIT_MOVQ_RAX_XMM0` 用 **R8** 作 scratch（`x86_64.c:866,914,1134`）；`EMIT_NUM_TO_XMM(src,scratch)` 用调用方给的 scratch（惯例 R8，`x86_64.c:998`）；`EMIT_CALL` 破坏所有易失寄存器（`x86_64.c:1214`）；FFI 内联写路径借 RCX/RDX/R8/RAX，**不能借 RSI/RDI**（非易失且序言未存当时语境，`ops_return.inc:234`）。

**栈同步规则**：跳转路径**一律先 `TOS_SPILL` 再跳**；**跳转目标与循环头 `tos_live` 必须为 0**（`x86_64.c:701,1937`）；合并点直落路径若 `tos_live` ⇒ 先 `push rax` 再记 offmap（否则每轮漏 8 字节，`x86_64.c:1957`）；`vstack_top`（callout 看到的名字）= **运行期 RSP**（`ops_callout.inc:35`）。

**func_mode vs loop_mode 的 6 处差异**：不写回 locals（`x86_64.c:2214`）；不 pin 局部量（`:501`）；不发入口守卫（`:1786`）；禁用内联（`jit.c:399`）；**不支持反向步进**（`ops_loop.inc:54`）；`OP_TAIL_CALL` 仅 func_mode 支持（`ops_callout.inc:92`）。

### 15.4 callout 契约

- **失败信号只有一条通道**：`jit_callout_failed`（volatile int，`jit_callout.c:27`）⇒ 绝不写全局错误收集器，报错文本交解释器重放 ✓；codegen 侧每个可失败 callout 之后 `mov r8,&flag; test; jnz → bailout` 并清标志（`ops_callout.inc:44`）。
- **粘滞性**：进入循环/函数前**必须复位** ✗（`jit.c:817`、`jit.c:643`），否则一次真失败连锁拉黑（§8.53）。
- **多返回值交付**：发布区 `jit_fn_results[]` + `jit_fn_result_count`（`jit_priv.h:327`）；`jit_fastpath_deliver_multi` 要求 `jit_fn_result_count == rc`，不符 ⇒ 返回 0 回落 VM 重入（`jit_callout.c:108`）。
- ⚠ **返回个数复位**：单返回路径的机器码 `OP_RETURN` **必须把 `jit_fn_result_count` 复位为 1**（§8.137 真凶 ✗）；多返回路径写编译期常量 `rc`（`ops_return.inc:52,128`）。
- ⚠ **结果槽类型必须作废**：多返回交付把 raw 直接写回**实参槽** ✗，而 `vt[]` 里存的是**实参旧类型** ⇒ 必须 `VT_INVALIDATE_MULTI_RET(rc)` 保守清 UNKNOWN，或按**声明返回类型**打标（`VT_SET_MULTI_RET_TYPES`，只映射 `FLOAT→DOUBLE` —— int 含 bigint 不能映射成 INT48 ✗）（`x86_64.c:732,750`）。
- ⚠ **rc 守卫必须在调用之前**：不等 1 直接 bailout，否则解释器重放会让 callee 执行两次（`jit_priv.h:593`、`jit_callout.c:2455`）。
- 模块指针**编译期嵌入**（`jit_scan_get_module()`），不查运行时帧（`jit_priv.h:512`）；`OP_GET_CSTRUCT_DEF` **不做编译期解析**（运行时注册、同名重声明会覆盖，`jit_callout.c:1179`）。

### 15.5 已知缺口 / 未完成（按"是否会算错"排序）

| # | 缺口 | 现状 | 证据 |
| --- | --- | --- | --- |
| 1 | **func_mode + 含回边 + `rc≥2` 调用**（曾实测算错 ✗）| **2026-09-20 整理时补上拒收** ✓：`scan` 记 `has_multi_ret_call`（模块函数/RESOLVED 方法三处取静态 rc ✓）＋ `jit_compile_function` 见 `has_back_jump && has_multi_ret_call` 即 `return NULL` ✓（**保守兜底** ✓）。⚠ 但**旧证据已过时** ✗：上游 §8.137 真因修复后 `probe_funcloop_shapes.leno` 开/关开关**都全绿** ✓（实测逐字相同 ✓）⇒ 该组合是否真会算错**尚未证实** ✗；上游若确认无碍，撤掉此拒收可换回那部分 Phase C 收益（开关开时实测仍 −20% ✓）。真正仍红的是**顺序相关**的 `test_jit_gc_safepoint` ✗（与"回边+rc≥2"无关 ✓）| `jit.c`（拒收点）、`jit_priv.h`（字段）、`jit_scan.c`（3 处置位）、`x86_64.c:146` |
| 2 | **`EMIT_BAILOUT_SITE_NONOVF` 与守卫区间撞号** ✗ | `NONOVF(off) = -1000-off`，`off≥1000` 时落进 `<=-2000` ⇒ 被 `jit.c` 误判成"入口守卫失败"（⇒ 走错的重编策略 + 原因误报）| `x86_64.c:1094` vs `jit.h:83`、`jit.c:877` |
| 3 | `LENO_JIT_FUNCLOOP`（含循环函数体进函数级 JIT）下 **AES 算错** ⇒ `test_jit_gc_safepoint` 红（**确定性** ✗ 非顺序！）| **2026-09-20 重新取证，推翻旧结论** ✓（旧记"顺序相关：整套红/单跑绿" ✗ —— 运行器是 `_exec` **每文件独立进程**（`assert/run_tests.leno:68`），根本没有跨测试状态 ✓；单跑开开关得 `BUG5_BAD=72` ✓ 与整套逐字相同 ✓）。**确定性事实**：① 开关开 ⇒ 80 轮里 72 轮错 ✓，首个错值 `i=8` 且 `block/往返/arkey` 均为 true ✓、**先坏的是字符串往返** ✗；② 被函数级编译的是 9 个**带循环**的小工具函数 ✓（`gf_mul`/`add_round_key`/`sub_bytes`/`shift_rows`/`mix_columns`/`inv_shift_rows`/`inv_sub_bytes`/`inv_mix_columns`/`key_expansion` ✓）；③ 定点诊断（`build/_probe_sp.leno`，同轮内重复调用 ✓）：**先**做的 `aes_decrypt_block(ct)` 正确 ✓、**后**做同一个调用变 `c03ee7087180ea222056066f120c0dee` ✗ ⇒ **同轮内越跑越坏 = 有状态损坏** ✗（不是某指令算错 ✓），与"GC 安全点"主题吻合 ✓；④ 已排除：`bytes_to_hex`/`hex_to_bytes`/`str_to_block`/`block_to_str`/`pkcs7_pad` 全对 ✓；⑤ 诊断签名：反复出现 `BAILOUT site=33 RAX=0xffc0000000000000`（**NaN 标签值**出现在期望裸值的检查处 ✗）| 本文件 §15.5 #3；`assert/test_jit_gc_safepoint.leno:396-431` |
| 4 | §8.135 S1 只做"知道"：返回码 5（侧退出）/ 精确续跑 / S3 旁路 / S4 去拉黑均为设计稿 | 未动手 | 本文件第 11/13 节 |
| 5 | `OP_TAIL_CALL_NATIVE` 未支持 | 静态拒收 | 本文件第 12 节 |
| 6 | `OP_CALL` 非闭包 callee 交 VM 慢路径的"脏值"疑点（怀疑 `OP_INDEX` 慢路径 rsp 相对寻址偏移）| 未查清 | `jit_callout.c:2404` |
| 7 | 其它限制：`step==0` / float 步长 for 不可 JIT；JIT 递归深度上限 64；函数级中途 bailout 重跑整个调用（副作用不回滚）；内联体不支持闭包/upvalue | 有意拒绝 | 本文件第 12 节 |

### 15.6 职责边界（改代码前先看这张）

| 文件 | 该做 | 不该做 | 证据 |
| --- | --- | --- | --- |
| `jit.c` | 编译驱动、公共 API、循环/函数缓存、bailout 反馈（探测窗口 / 延迟释放 / `guard_disabled` / `avoid_fold`）| 任何 ISA 相关发射 | `jit.c:1,228,502,689` |
| `jit_scan.c` | `cache_hash`、`opcode_size(_chunk)`、`scan_loop_body`（栈深走查 + 拒收判定 + 内联分析）、`jit_gaps` 直方图、编译期解析 | 发射机器码 | `jit_scan.c:1,1024` |
| `jit_callout.c` | 全局状态（`jit_callout_vm/_failed`、`jit_fn_result*`、深度、yield 标志）+ 所有运行时 C helper + 延迟释放队列 | 决定"编不编" | `jit_callout.c:17,297` |
| `backend/x86_64.c` | 发射骨架：序言 / locals / macro / while(op) / 出口 / bailout / 桩 / patch，以及 `TOS_*`·`EMIT_*` 宏 | 编译策略判断（那种东西属于 `jit.c`/`jit_scan.c`）| `x86_64.c:421,2046` |
| `backend/x86_64_emit.h` | 寄存器 ID + 纯编码器 `emit_*` | 被 `ops_*.inc` 之外的文件包含 | `x86_64_emit.h:1` |
| `backend/x86_inc/ops_*.inc` | 按 opcode 家族实现 case（`#include` 进 `switch(op)`，**不是独立编译单元**）| 定义全局状态 / 新寄存器约定 | `x86_64.c:2046` |

### 15.7 整理用的门禁（2026-09-20 实测基线，改完必须复测）

| 门禁 | 命令 | 基线 |
| --- | --- | --- |
| 全量用例 | `build\leno.exe assert\run_tests.leno` | **335 passed / 0 failed** ✓ |
| JIT 形状/最小复现探针 | `jit_probes\probe_funcloop_shapes.leno`、`probe_funcloop_minret.leno`、`probe_array_literal_float.leno` … | 全绿 ✓ |
| 性能（光线追踪）| `build\leno.exe examples\性能测试\光线追踪对比.leno` | `hits=585500` ✓；**本机 2026-09-20 实测**：Phase A ≈97ms、Phase B ≈578ms、B2 ≈579ms、**Phase C ≈2612~2691ms**（3 次方差 <3%）；同时 `Bailouts: 0`、`Compiled: 5`、`FuncCompiled: 2` ✓ |
| JIT 覆盖普查 | `jit_probes\jit_census.ps1` | 出直方图 ✓（看 `func|函数体含循环` 等拒收理由）|

> ⚠ 本机慢 ⇒ 门禁里最贵的是全量用例 ✓；**改 JIT 必须至少跑全量用例 + 光线追踪** ✗（这两条能抓住：错值、以及"没变快但变慢"）。
>
> ⚠ 性能数字只能跟**本机自己**的历史比 ✗：上游提交里记的 Phase C ≈1003ms（以及 `LENO_JIT_FUNCLOOP=1`
> 下的 773ms）是**别的环境**的数字 ✓ —— 本机实测 ≈2612ms，整体慢 ≈2.6× ✓。
> 本节数字就是为"以后有本机基线可对比"而立 ✓（2026-09-20 整理时首次记录本机数字 ✓）。

### 15.8 写入者–作废者矩阵（改一行前先回答这三个问题）

> **改任何与类型/栈有关的地方，先答**：① 谁写它？② 谁作废它？③ 它是"一次性"还是"持续有效"？
> 答不上来就先别改 ✗ —— §8.127 折叠两次失败、`TOS_SPILL` 抹掉刚标的 DOUBLE，都是漏了这三问 ✓。

| 数据 | 作用域 | 写入者 | 作废者 | 读者 | 一次性？ |
| --- | --- | --- | --- | --- | --- |
| `vt[]` | 每个**栈槽**（`vstack` 域） | 见 15.1 的 9 处（`TOS_SPILL` / `TOS_PRODUCE*` / `VT_*_MULTI_RET` / 合并点求交）| 跳转目标、**回边目标**（整块清）、合并点求交不一致、每槽消费、内联返回 | `OP_ARRAY` 静态通道、`OP_CAST_INT` 省略、`EMIT_RAW_TO_VALUE_T` 等 | 否（持续，直到被上面任一事件清掉）|
| `tos_pending` | 只对**下一条产出**有效 | 产出指令**在 `TOS_PRODUCE()` 之前**设（int48 检查 / 浮点产出 / `OP_GET_LOCAL` 传播）| `TOS_PRODUCE*` 取走即清 ✓；每条指令开头再兜底清一次 ✓ | `TOS_PRODUCE` / `TOS_PRODUCE_FROM_RDX` | **是** ✓（未标注的产出自动 UNKNOWN，绝不继承陈旧值 ✗）|
| `local_vt[]` | 每个**局部量 scratch 下标 si**（编译期）| `LOCAL_VT_SET`：INC/DEC 六族 + `SET_LOCAL*`/`MOVE_LOCAL*`（白名单 ✓）| 合并点**只清"循环体内被写过"的槽** ✓、内联返回整块清 ✓；不在白名单的写点 ⇒ 该槽在 `local_untracked_mask` 里 ⇒ 一律 UNKNOWN | `OP_GET_LOCAL`（传播到 TOS）、`OP_GET_FIELD_FAST` 慢路径装箱 | 否（但要先过"白名单 + 非内联 + si<64"三关 ✓）|
| RBX 位图 | 每个局部量（**运行期**）| 序言逐槽 tag 测试建立 ✓ | 循环体内**写过**该局部量 ⇒ 位图是入口快照会陈旧 ✗（`local_written_mask` 拦 ✓）| `OP_ARRAY` **动态通道**（`BT RBX, si`）| 否（整段入口快照 ✓）|
| `slot_from_si` | 每个栈槽（"本槽来自哪个 scratch 读"）| 产出指令标 | 跳转目标 / 净推 ≤ 0（顶槽可能被覆盖 ⇒ 先清零 ✓）| 动态通道（据此决定要不要发 `BT RBX`）| 否 |
| `OP_SET_PTR_ELEM_TYPE` / `OP_SET_DECLARED_FACE` | **对象自身字段**（`ptr->element_type` / `struct->declared_face`）| 这两条 opcode 自己 ✓ | 对象生命周期 | 解释器与 JIT 的后续装箱 | 否（**不碰 `vt[]`/`local_vt` ✓、不弹不推 ✓**）|

**下一步（#3 的既定方向，按性价比排序）**：

1. **`LENO_JIT_FUNC_CACHE_SMALL=1`（确定性撞槽开关，`jit.c:502`）** ⇒ 把 1024 槽缩到 4 槽：若故障形态改变/加剧 ⇒ 病灶在**函数缓存驱逐 + 延迟释放**一族 ✓（"越跑越坏"很像被驱逐的机器码 / 复用的槽位 ✗）；
2. **核对 func 出口路径的 `jit_func_depth` 与"是否仍在机器码里"是否始终一致** ✗ —— 怀疑点是**回边让出（返回码 4）**与 **bailout（返回码 1）** 的出口：若某条出口让 `jit_func_depth` 提前归零，而**调用方的 `flocals` / 机器栈仍持有活值** ✗ ⇒ `jit_in_frame()` 返回 0 ⇒ GC 就地回收 ⇒ 正是"越跑越坏" ✓（对应测量：出口后仍有活值的窗口 ✗）；
3. **逐个函数排除** ✓：`LENO_JIT_NOINLINE=1` 先把内联变量去掉 ✓，再用探针只调用单个 helper（配绝对期望值 ✓）逐个定位 ✗。

**2026-09-20 已做的三组实验（结论：排除 1 与 3，锁定"过程中的编译事件"）**：

| 实验 | 结果 | 结论 |
| --- | --- | --- |
| A 基准 `FUNCLOOP=1` | `BUG5_BAD=72`，首错 i=8 | 基准 ✓ |
| B `+FUNC_CACHE_SMALL=1` | `70`，首错 i=**10** | 只位移了首错点 ⇒ **不是缓存撞槽/驱逐** ✗ |
| C `+NOINLINE=1` | `72`，首错 i=8 | 与内联无关 ✗ |
| D **预热实验**（先调 200 次再跑回归）| **全对 ✓** | ⚠ **该实验无效 ✗（2026-09-20 自我更正）**：当时把回归缩到 **3 轮** ✗ ⇒ `aes_decrypt_block` **根本没到编译阈值**（实测第 **49** 次才被编译 ✓）⇒ "全对"是同义反复 ✗。由它推出的"不是 helper 本身算错"**作废** ✗（见第四次更新 ✓）|
| E **`_gc(false)` 关 GC**（内建 `_gc(false)` ✓，探针里用 `PROBE_GC_OFF=1` 控制 ✓）| **`BUG5_BAD=72`**，与 GC 开**逐字相同** ✗（`LENO_NO_JIT` + 关 GC ⇒ 0 ✓ 对照）| ⇒ **GC 就地回收被排除** ✓✓ —— 不是"JIT 帧的活值被当垃圾回收" ✗ |
| F **两套缓存并存**（`LENO_JIT_CLOG=1` + `LENO_JIT_DEBUG=1` 同开 ✓）| 日志里**同一个函数**既有 `func compiled: 'gf_mul' …` ✓ 又有 `[CLOG] loop fn='gf_mul' body_bc=51 back_edge=2 compiled=1` ✓（`key_expansion` 两段 body 各一条 ✓）| ⇒ **假设成立** ✓：同一段 body 同时躺在**函数缓存**与**循环缓存**里 ✓（两套约定互斥 ✗）|
| G **循环级守卫**（`jit_compile` 入口：body 落在"已被函数级编译"的 chunk 内 ⇒ 不做循环级 ✗）| **完全无效** ✗：`loop fn=…` 仍出现 5 条 ✓、守卫的 `jit_gaps_record` 一条都没记 ✓ | ⇒ 自然顺序里**循环编译在先**（那时函数还不在函数缓存里 ✓）⇒ 该方向不可能触发 ✗（已回退 ✓）|
| H **函数级编译成功后作废体内 loop 条目**（`jit_code_retire` 延迟释放 ✓，`LENO_JIT_KEEP_DUAL=1` 对照 ✓）| **两组都是 72** ✗（对照无任何差别 ✓）| ⇒ "体内残留 loop 条目"**也不是**病灶 ✗（已回退 ✓）|

⇒ **F 只证明了"共存"这个事实 ✓，G/H 证明它**≠因果** ✗** —— 两条尝试都无效 ✓，已全部回退 ✓（`jit.c` 与 HEAD 零差异 ✓，开关关时 `passed` ✓）。

**当前首选机理假设（待验证 ✓）**：**同一段 body 被两套缓存各编了一份** ✗ ——
函数体被**解释执行**时，它内部的循环会进**循环缓存**（键 = `cache_hash(body_start)` ✓）；
`LENO_JIT_FUNCLOOP=1` 之后，**同一段 body** 又能被当**函数**编进**函数缓存**（键 = `jit_func_cache_slot(func)` ✓）。
两套机器码**调用约定不同**（func_mode：不写回 locals、`OP_RETURN` 直返 ✓；loop_mode：写回 locals、出口块 ✓）⇒
一旦"该进 A 的进了 B"就是静默错值 ✓。该假设同时解释了三条实测 ✓：
预热 ⇒ 编译发生在解释器还在顶层时 ⇒ 不产生"两套并存" ✓；
`FUNC_CACHE_SMALL` ⇒ 只改变"何时编译" ⇒ 只位移首错点 ✓；
`NOINLINE` 无关 ✓。

**验证该假设的最短路径**：在 `LENO_JIT_FUNCLOOP=1` 下给两个缓存加一行日志（`loop: body_start=%p compiled / func: func=%p body_start=%p compiled` ✓），
跑本测试看**同一 `body_start` 是否同时出现在两张表** ✓；若是 ⇒ 修法是"函数级编译成功时作废该 body 的循环缓存条目"（或反之 ✓），
并写进 §15.6 的职责边界 ✓。

**2026-09-20 更新：共存已由实验 F 证实 ✓（不必再加日志 ✓），剩下的是"因果"** ✗ ——
下一个**决定性实验**（最小改动 ✓）：在 `jit_try_hot_loop` 的入口拒绝"**所属函数已被函数级编译**"的 body
（实现：按 `body_start` 从 `jit_func_cache` 反查 `e->func->chunk->code == body_start` ✓，冷路径 ✓ 开销无所谓 ✓），
然后重跑本测试 ✓：
- 若**故障消失** ⇒ 因果确认 ✓ ⇒ 这就是**修法**（函数级与循环级对同一 body 二选一 ✓），可继续试把开关转正 ✓（−20% ✓）；
- 若**故障仍在** ⇒ 病灶不在"两套并存"，回到"编译侧副作用"路线 ✓（下一嫌疑：编译是否改写了 `func->chunk->code` / `return_count` 等被后续执行依赖的持久状态 ✗）。

**2026-09-20 第二次更新（G/H 证伪后，剩下两个嫌疑，按证据强弱排序）**：

1. **函数级中途 bailout ⇒ 解释器重跑整个调用 ⇒ 副作用重复** ✗（本文件 §12 早就记着"函数级中途 bailout 重跑整个调用，
   堆侧副作用不回滚 ✗"，`§14` 也列过）。**证据**：诊断日志里反复出现
   `[JIT-DEBUG] BAILOUT site=33 RSP=… RAX=0xffc0000000000000` ✓ —— 而 `site=33` 是**正数** ✓
   = 溢出/截断类检查命中 ✓ ⇒ 该函数级调用**中途被打断** ✓。预热之所以全对 ✓，可能正是因为
   预热后**不再 bailout**（或 bailout 点不再落在关键轮次 ✓）；
2. **编译侧副作用**：编译是否改写了 `func->chunk->code` / `return_count` 等**被后续执行依赖**的持久状态 ✗
   （预热前编 vs 预热后编的差别 ✓ 也可能来自这里 ✓）。

**下一步（最短 & 最可能一击命中 ✓）**：把失败配置的**合并流日志**按时间对齐 ✓
（`cmd /c "leno.exe test > both.log 2>&1"` ✓，能把 stderr 的 `[JIT-DEBUG] BAILOUT …` 与 stdout 的
`[GC-SAFEPOINT] i=…` 交错记录 ✓），只看**第一次 `i=8` 之前最后几条 BAILOUT** ✓：
- 若紧邻的 BAILOUT 属于**被调用的 AES helper** ✓ ⇒ 嫌疑 1 成立 ✓ ⇒ 修法是给函数级 bailout 加"已发布结果 /
  已产生堆副作用的写回点"语义（或干脆让**含堆副作用的函数不进函数级 JIT** ✗ 保守版 ✓）；
- 若没有任何 BAILOUT ✓ ⇒ 嫌疑 1 排除 ⇒ 转嫌疑 2（对比预热前后 `func->chunk` 的字节）✓。

**2026-09-20 第三次更新（合并流日志对齐 ⇒ 首次错值与"某函数的函数级编译时刻"重合 ✓）**：

用 `cmd /c "leno.exe test > both2.log 2>&1"` 得到**交错**的时间线 ✓（35772 行 ✓）：
`BAILOUT` 共 **5560** 条 ✓，`site=33` **4813** 条（正数 ⇒ 溢出/截断类命中 ✓）、`site=36` 374、`site=291` 341、`site=137` 31 ✓。
第一次 `[GC-SAFEPOINT] i=8` 出现在第 **7764** 行 ✓，它**之前紧邻**的事件是：

```
[JIT-VS] RETURN 'aes_decrypt_block' bc=705 / bc=707 vstack=0/1 tos_live=1 type=0
[JIT-DEBUG] func compiled: 'aes_decrypt_block' func=… slot=0 locals=20 bytes=708   ← 恰在此刻被函数级编译
[JIT-DEBUG] EXEC call #1427 … fn=… locals=…      ← 随后连续进入机器码（#1427~#1431 …）
[JIT-DEBUG] EXEC returned 0
[JIT-DEBUG] post-JIT: frame_cnt=5 ip_off=68 sp=6
[GC-SAFEPOINT] i=8 结果错: block=true 往返=true arkey=true 字符串=false            ← 首个错值
```

⇒ **首个错值与"`aes_decrypt_block`（含 `for 9:1:-1` 的函数）被函数级编译"的时刻重合** ✓，
且此后 `site=33` 的 bailout 呈**压倒性多数**（4813 次 ✓）⇒ 该函数在整个过程中**在机器码与解释器之间反复交替** ✗。
结合"预热后全对 ✓"与"同一轮内先对后错 ✓" ⇒ 嫌疑收敛为：
**函数级 JIT 与解释器交替执行同一函数时的"交付/复位"语义** ✗（`jit_fn_result` / `jit_fn_result_count` /
`jit_callout_failed` / 出口写回 与"bailout 后解释器重跑"之间的衔接 ✓），而**不是**编译产物本身（预热已证 ✓）。

**最小复现（下一步，约 10 分钟 ✓）**：改 `build/_probe_sp.leno` 的 `main` —— **预热除 `aes_decrypt_block` 之外的全部 helper** ✓，
然后只调用它若干次并打印结果 ✓ ⇒ 若**第一次调用就错** ⇒ 复现缩到"单独一个函数 + 一次调用" ✓，
比 80 轮 torture 好调试得多 ✓；再在该最小例上对照 `_gc(false)` / `LENO_JIT_NOINLINE` / `FUNC_CACHE_SMALL` ✓ 复核一遍 ✓。

**2026-09-20 收口（最小复现 + 修复 ✓）**：

**最小复现**（`build\_probe_sp.leno` ✓）：只预热**除 `aes_decrypt_block` 之外**的 8 个 helper ✓，
然后连调它 300 次并比对绝对期望值 ✓：

| 配置 | 结果 |
| --- | --- |
| `FUNCLOOP=1` | **第 49 次调用开始错** ✗ `c03ee7087180ea222056066f120c0dee`（期望 `3243f6a8885a308d313198a2e0370734` ✓）；第 1~48 次全对 ✓ |
| `LENO_NO_JIT=1` | 300 次**全对** ✓ |

⇒ **49 ≈ 函数级热度阈值** ✓ ⇒ 结论回到文档最初的判断、并被**实测坐实** ✓：
**`aes_decrypt_block` 的 `for 9:1:-1`（反向步进）在 func_mode 下被编出错值** ✗，
且**机器码从第一次执行就错**（不是"渐进损坏" ✗、不是 GC ✗、不是缓存 ✗）。
此前几条"排除 helper 本身"的结论都建立在实验 D 之上 ⇒ **一并作废** ✗（D 行已更正 ✓）。

**修复**（`jit.c` 的 `func_body_is_simple` ✓，保守版 ✓）：原来只在"步进常量**已知为负**"时拒收 ✗ ——
而实测该 `-1` **不是可见常量** ✗（`have_const=0` ✓，§8.137 早记着这点 ✓）⇒ 那道门**兜不住** ✗。
现在改为：**步进方向能证明非负才放行** ✓（"不可知"也拒 ✓）。
...
| --- | --- |
| 最小复现（开关开）| 300 次**全对** ✓ |
| `test_jit_gc_safepoint`（开关开）| **passed** ✓（之前 `BUG5_BAD=72` ✗）|
| 默认门禁（开关关）| **335 / 0** ✓ |
| 默认光线追踪 | `hits=585500` ✓、Phase C **2647ms** ✓（基线 2612~2691ms ✓ 无回归 ✓）|

**代价（如实 ✓）**：开关开时 `FuncCompiled` 3 → 2 ✓ ⇒ `trace`（步进是变量但恒正 ✓）也被拒 ✗
⇒ 光线追踪 Phase C 的 **−20% 暂时拿不到** ✗（2746ms vs 默认 2647ms ✓）。
**下一步（要拿回 −20% ✓）**：给 func_mode 的 `OP_FOR_PREP` 加**运行期守卫** ✓
（序言处检查步进 ≥ 0 ✓，不满足就 bailout ✓ —— 复用已有的 `CodegenCtx.no_guard` 机制 ✓），
这样**正向循环仍进机器码** ✓、反向循环安全退回解释器 ✓，两全 ✓。

**两条最容易踩的推论**：

1. `local_vt[]`（编译期）与 RBX 位图（运行期）**不是一回事** ✓ —— 前者问"编译期能不能证明这个 scratch 槽的类型"，后者问"运行期这个局部量是不是 int48"。想用 RBX 位图，必须保证**循环体内不写该局部量**（否则是入口快照 ✗）。
2. 任何"跨合并点仍成立"的证明，**不能**只靠 `vt[]` ✗（通道在合并点会保守失效 ✓）—— 要么走 `local_vt[]`/位图这类"按槽且不随控制流清空"的通道 ✓，要么先让通道本身路径敏感（S2/S3 做的事 ✓）。
