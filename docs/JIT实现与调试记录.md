...
**已知成本与后续**：轮询给「循环体内有 callout」的循环每轮加 2 条指令。对真正在分配的
循环（每轮本来要付一次 ~200ns 的 callout）可忽略；但**纯读循环**（`arr[i]` / `dict[k]`
也是 callout）会白付这份钱 —— 实测 `arr[index]` 1 亿次从 187ms 级升到 203~219ms。
两个后续方向（都还没做，当前基准全部落在既有噪声带内，暂无证据说明必须做）：
① 更细的门：只认**直接分配**的 opcode 与**内联点**，把纯读 callout 排除；
② 把标志地址预装进 callee-saved 寄存器（R15），轮询降到 1 条指令。
