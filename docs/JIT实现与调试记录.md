...
| 20 | inline | **`OP_CALL`(59) 内联扫描没有 case** | **可做且便宜**：照 loop scan 的同名 case 补记账 |
...
"跨模块访问" **48**（刻意保留）⇒ **真正"补 opcode 就能拿下"的只剩内联侧 `OP_CALL`(20)
与 `OP_DICT`(3)**。覆盖面工作基本收尾：剩下的要么是刻意保留 / 设计限制，要么需要按 FFI 单独立项。
