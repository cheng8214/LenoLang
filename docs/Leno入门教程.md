...
### int 固定位宽函数 `_int32()` / `_int64()` / `_uint32()`

LenoC 的 `int` 是任意精度大整数，永远不会溢出。但在加密算法、网络协议、文件格式等场景中，经常需要固定位宽的环绕运算（overflow wrapping）。`_int32()`、`_int64()` 和 `_uint32()` 提供了这种能力。
...
> **⚠️ 注意：`_int32()`/`_int64()`/`_uint32()` 与 `_int()` 的区别**
>
> | 写法 | 语义 | 结果 |
> |------|------|------|
...
>
> `_int()` 是类型转换，不改变数值；`_int32()`/`_int64()`/`_uint32()` 是位宽截断，会环绕。
...
> _int32(_uint32(x) << c | _uint32(x) >> (32 - c))  // 32 位循环左移
> ```
...
>
> // 如需逻辑右移（高位补 0），使用 _uint32 转为无符号值：
> _uint32(x) >> 1       // 逻辑右移，高位补 0
> ```
