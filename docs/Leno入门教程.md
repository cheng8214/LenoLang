...
### int 固定位宽函数 `_int32()` / `_int64()`

LenoC 的 `int` 是任意精度大整数，永远不会溢出。但在加密算法、网络协议、文件格式等场景中，经常需要固定位宽的环绕运算（overflow wrapping）。`_int32()` 和 `_int64()` 提供了这种能力。
...
> **⚠️ 注意：`_int32()`/`_int64()` 与 `_int()` 的区别**
>
> | 写法 | 语义 | 结果 |
> |------|------|------|
> | `_int(3.14)` | 强制类型转换 | `3`（截断小数） |
> | `_int32(3.14)` | 截断为 32 位整数 | `3` |
> | `_int32(0x1FFFFFFFF)` | 32 位环绕 | `-1`（低 32 位） |
> | `_int(0x1FFFFFFFF)` | 保持原值 | `8589934591`（不截断） |
>
> `_int()` 是类型转换，不改变数值；`_int32()`/`_int64()` 是位宽截断，会环绕。
...
| 数组比较  | `[1,2] == [1,2]`                              |
| 类型转换  | `_int(x)`, `_float(x)`, `_str(x)`, `_bool(x)`, `_int32(x)`, `_int64(x)` |
