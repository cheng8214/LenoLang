...
11. [解构声明](#解构声明)
12. [字符串](#字符串)
    - [易错点：$ 只作用于紧跟它的字面量](#易错点-只作用于紧跟它的字面量)
13. [模块系统](#模块系统)
...
⚠️ **插值不是"整行生效"的**，它只修饰 `$` 后面紧跟的那个 `"..."`。一旦你把它和 `+` 拼接混用，
**后面的普通字符串里的 `{...}` 不会被替换，而是原样输出**：
...
main() {
    ...
    // ❌ 错误：只有第一个字面量是插值字符串，第二个是普通字符串
    print($"共 {cnt} 个  " + total + "ms 结果:{total}\n")
    // 输出: 共 100 个  500ms 结果:{total}        ← {total} 没有替换！

    // ✅ 写法 1（推荐）：整段都放进一个 $"..." 里
    print($"共 {cnt} 个  {total}ms 结果:{total}\n")

    // ✅ 写法 2：拼接时显式转换
    print($"共 {cnt} 个  " + _str(total) + "ms 结果:" + _str(total) + "\n")
}
```

> **为什么特别容易误判**：`{cnt}` 生效了、`+ total +` 那段的数字也"看着正常"，
> 于是很容易以为插值工作正常。其实前者是插值、后者是 `+` 的**自动转字符串**（`OP_STRING_ADD`），
> 唯独最后那个普通字面量里的 `{total}` 是死的。
>
> **口诀**：**同一个输出里，要么整段放在一个 `$"..."` 内，要么对每个变量显式写 `_str(x)`。**
> 另外注意：普通字符串里出现 `{` 不报错（它就是个普通字符），所以**编译器不会提醒你**，
> 只能靠这条习惯规避。
>
> **插值里可以写任意表达式**（含运算符和空格）：`$"总耗时 {t2 - t1}ms"` 是合法的。
...
>
> - 确保分配的内存正确释放，避免内存泄漏
> - **GC 兜底**：FFI 资源（`ffi.malloc`/`ffi.load`/`ffi.callback`）由 GC 自动追踪。忘记 `ffi.free()` 时，对象不再被引用后 GC 会自动回收底层 C 内存/句柄/回调，不会泄漏。但 GC 回收时机不确定，推荐显式 `ffi.free()` 精确控制生命周期
...
| 字符串插值 | `$"Hello {name}"`（⚠️ `$` 只作用于紧跟的那个字面量，与 `+` 混用见 [易错点](#易错点-只作用于紧跟它的字面量)） |
| 原始字符串 | `@"raw string"`                               |
