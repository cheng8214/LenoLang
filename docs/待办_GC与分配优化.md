...
> **更新（2026-09-15，§8.74）**：上表的绝对值是 i5-3450 时代的；本机（同构建 A/B）现在
> `不分配 11.9 ns / new Pair 110 ns / new Big 170 ns / 数组字面量 149 ns`。
> 更重要的两点修正：
> 1. **「每字段 +~15ns」与「~79ns 未归因」主要不是"字段写入"本身**，而是**冷内存**代价
>    （从不复用的内存要 memset + page fault + cache miss）。池能复用之后，
>    边际每字段成本从 ~14.9 ns 掉到 **~4.7 ns**（Pair→Big 的差 ÷ 4）。
> 2. 因此**分配路径的第一优先级已变成"让池吃饱"**（已做：默认 32MB）——
>    这一项就拿回 −38%/~−47%，比任何字段级微优化都大。
> 下一步的分配侧优化空间只剩：cold path 的 `memset(0)` 是否可省（需先证安全）、
> `OBJ_ARRAY` 的复用、以及 callout 包装那 ~14 ns。
...
2. ~~❌ **把 `GC_POOL_MEM_LIMIT` 从 4MB 放大到 16/32MB**。§8.31 已测：64MB 只省 ~4 ns。
   ...
   所以小幅度放大只是换个地方撞墙。~~
   **✅ 本条目已被推翻（2026-09-15，§8.74）—— 不要再把它当负结果。**
   它的前提是"JIT 循环内对象**不会**回收到 free-list"；而 §8.37/§8.38 落地**回边让出**
   之后回收会周期性发生 ⇒ 池只需覆盖**一个让出间隔**内的分配量（实测每 ~3 万次分配
   让出一次 ⇒ 约 2.4MB 起），不再是"整段"。重新实测（同机、同构建，
   用新加的 `LENO_GC_POOL_LIMIT` 切配置，`jit_probes/probe_alloc2.leno`，两轮噪声 ±2ns）：

   | 池上限 | `new Pair`(2 字段) | `new Big`(6 字段) |
   | --- | --- | --- |
   | 4MB（旧默认） | **110.0 / 107.2 ns** | **170.2 / 168.5 ns** |
   | 16MB | 69.9 ns | 95.5 ns |
   | **32MB（新默认）** | **68.1 / 69.3 ns** | **88.8 / 90.1 ns** |
   | 64MB | 69.9 ns | 90.4 ns |
   | 1024MB | 67.8 ns | 87.6 ns |

   ⇒ **−38% / −47%**，**32MB 即饱和**（16MB 略欠）。默认值已改为 32MB；
   上限只是**天花板**（池按需增长），小程序的常驻内存不受影响。
   （`数组字面量[1.0,2.0]` 不受影响：`OBJ_ARRAY` 有意排除在池外 —— 它有自己的
   header 回收链，`arr_new` 复用时会清 `flags`，双重持有会破坏池链，
   见 `gc.c` 的池设计注释。）
