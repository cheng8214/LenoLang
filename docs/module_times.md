...
单调计时（毫秒，带小数）。与 `us()`/`ns()` 共用同一高精度时钟源（Windows QPC / POSIX CLOCK_MONOTONIC），**精度由时钟源决定、与单位无关**。

**参数**: 无  
**返回**: `float` - 单调毫秒计数（带小数；`int` 会把毫秒截断成 1ms 粒度，所以不返回 int）
...
| 方法 | 精度 | 用途 | 示例值 |
|------|------|------|--------|
| `ms()` | 毫秒（带小数） | 一般计时 | 123456.789 |
| `us()` | 微秒（带小数） | 高精度计时 | 123456789.0 |
| `ns()` | 纳秒（带小数） | 超高精度计时 | 123456789000.0 |
| `now()` | 秒 | Unix 时间戳（挂钟） | 1777650164 |

> `ms()`/`us()`/`ns()` 都是**单调计数**（示例值是"自开机以来的计数"量级，不是日期），
> 只用于取差值；需要日期/时间戳请用 `now()` / `format()` / `datetime()`。
...
- **计时把量程拉长**：单项负载跑到 ≥100ms（或重复多轮取最小值）。
  短负载的分辨率与抖动会淹没差异 —— 例如 15ms 的负载在 15.625ms 粒度下会变成"1 个刻度"的假数
...
1. **计时分辨率与量程（2026-09-16 修正）**
   - `ms()`/`us()`/`ns()` 共用同一高精度单调时钟源（Windows QPC / POSIX CLOCK_MONOTONIC）：
     **精度由时钟源决定，与单位无关** —— 换算成毫秒不丢精度，所以 `ms()` 也返回 `float`
   - 三者都是**单调计数**，只用于取差值；挂钟时间请用 `now()` / `format()` / `datetime()`
   - **量程比分辨率更容易毁掉测量**：单项负载请跑到 ≥100ms，或重复多轮取最小值，
     否则调度/缓存/频率抖动会让数字不可复现
   - 历史问题：`ms()` 曾用 `GetTickCount64`（分辨率 ≈15.625ms）且返回 `int`，
     短测量会出现"整刻度"假象（如 16ms 实为 1 个刻度，真值可能只有 0.4ms）
