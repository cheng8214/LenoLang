...
## 大整数语义门禁（§8.123 / §8.124 / §8.125）

- **`probe_bigint_const_ops.leno`**（§8.125）—— bigint × **常量**运算符（走 IMM / 折叠路径 ✓）：
  `>> 32` / `<< 4` / `& 0xffffffff` / `| 0x8000…` / `^ 0xff…` + 三个比较 ✓。
  判读：必须与 `LENO_NO_JIT=1` 逐字一致 ✓；期望值见文件头（手算 ✓）。
  **为什么单列**：通用移位/位运算一直有操作数检查 ✓，只有 **IMM / 折叠**形态漏了 ✗
  ⇒ 症状特征极其好认："**只有常量运算数坏、动态的没事**" ✗。


- **`probe_bitwise_bigint_sign.leno`** —— `&`/`|`/`^` 在**大整数 + 负操作数**下的符号与幅值
  （8 行真值表：`+2^63 & 2^64-1`、`-1 & mask`、`-1 ^ mask` = `-2^64`、`-2 & -3`、混合符号 … ✓）。
  判读：必须与 `LENO_NO_JIT=1` **逐字一致**，并等于 **Python 语义** ✓
  （`(1<<63) & (2**64-1)` = 9223372036854775808 ✓、`-1 ^ (2**64-1)` = -18446744073709551616 ✓）。
  ⚠ 这组用例要**一行一行手算** ✓：§8.124 当时是两个缺陷叠加（符号判据 + 补码转换 ✗），
  只修一个会让结果"看起来好了一半" ✗。
- `_jit_bugs/` 下另有用户提交的复现件（`bug1`~`bug4` ✓）与两份切分诊断
