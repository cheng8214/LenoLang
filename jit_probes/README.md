...
| `probe_dict_set.leno` | 字典写的基线与回归锚点（§8.70 负结果）：`d["key"]=n`（命中已有条目）、`d["obj"]="value"`（对象值）、每轮新键（回退路径）、bigint 值、链式累加。`n` 由命令行给出（**计时必须 ≥2000 万** —— `times.ms()` 刻度约 16ms）。用途：① 记录 JIT / `LENO_NO_JIT=1` 的字典写基线；② 盯住**未解释的现象**：`new` 组在 JIT 下比解释器慢 5~10 倍（§8.70 副产品 2）。 |
